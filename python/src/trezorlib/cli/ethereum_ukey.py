...
from .. import ethereum_ukey as ethereum, tools
...
) -> List[ethereum.messages.EthereumAuthorizationUKey]:
    try:
        return [_parse_authorization_list_item(val) for val in value]
    except Exception:
        raise click.BadParameter("Authorization List format invalid")

def _parse_authorization_list_item(value: str) -> ethereum.messages.EthereumAuthorizationUKey:
    try:
        arr = value.split(":")
        ...
        return ethereum.messages.EthereumAuthorizationUKey(
...
def _format_authorization_list(
    authorization_list: List[ethereum.messages.EthereumAuthorizationUKey],
...
@click.group(name="ethereum-ukey")
def cli() -> None:
    """Ethereum UKey commands."""
...
def sign_tx(
    ...
    if is_eip1559:
        ...
        signed_authorization_list = [
            ethereum.messages.EthereumAuthorizationUKey(
