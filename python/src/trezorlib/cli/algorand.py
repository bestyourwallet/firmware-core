...
PATH_HELP = "BIP-32 path, e.g. m/44'/283'/<account>'/0/0"
...
def get_address(client: "TrezorClient", address: str, show_display: bool):
    """Get Algorand address for specified path."""
    address_n = tools.parse_path(address)
    return algorand.get_address(client, address_n, show_display)
...
def sign_raw_transaction(client: "TrezorClient", address: str, message: str):
    ...
    address_n = tools.parse_path(address)
    ret = algorand.sign_tx(client, address_n, bytes.fromhex(message))
    output = {
