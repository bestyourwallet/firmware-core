...
def is_set() -> bool:
    if utils.USE_ACL16_WALLET_CRYPTO:
        auth_type = se_acl16.authorization_get_type()
...
def set(auth_message: protobuf.MessageType) -> None:
    ...
    if utils.USE_ACL16_WALLET_CRYPTO:
        se_acl16.authorization_set(auth_message.MESSAGE_WIRE_TYPE, buffer)
    else:
        storage.cache.set(
...
def get() -> protobuf.MessageType | None:
    if utils.USE_ACL16_WALLET_CRYPTO:
        ...
        buffer = se_acl16.authorization_get_data()
    else:
