...
from trezor.lvglui.scrs import fingerprints, lv_theme, theme_path_png
...
async def request_pin(
    ...
    **kwargs: Any,
    ...
    return await request_pin_on_device(
        ...
        nav_back=kwargs.get("nav_back", False),
    )
...
async def pin_mismatch(ctx) -> None:
    ...
    await show_warning(
        ...
        btn_yes_bg_color=lv_theme.BTN_YES_BG,  # lv_colors.UKEY_O_BLACK,
    )
...
async def passphrase_pin_used(ctx: wire.Context):
    screen = FullSizeWindow(
        ...
        _(i18n_keys.TITLE__PIN_ALREADY_USED_DESC),
