    ...
    def __init__(self, seed: bytes | None = None, data: bytes | None = None) -> None:
        if utils.USE_ACL16_WALLET_CRYPTO:
            if data is not None:
                ...
                self.data = se_acl16.slip21_node()
        else:
...
if not utils.BITCOIN_ONLY:
    ...
    async def derive_and_store_roots(ctx: wire.Context) -> None:
        ...
        if not utils.USE_ACL16_WALLET_CRYPTO:
            ...
            need_cardano_secret = not cache.is_set(cache.APP_CARDANO_ICARUS_SECRET)
            ...
            if not passphrase_pin_enabled:
                ...
                current_passphrase_str = se_acl16.get_pin_passphrase_current()
                passphrase_str = (
                    current_passphrase_str if current_passphrase_str is not None else ""
                )
            ...
            if not state[0] & 0x80:
                ...
                if not passphrase_pin_enabled:
                    ...
                    current_passphrase_str = se_acl16.get_pin_passphrase_current()
                    passphrase_str = (
                        current_passphrase_str
                        if current_passphrase_str is not None
                        else ""
                    )
...
def derive_slip21_node_without_passphrase(path: Slip21Path) -> Slip21Node:
    if utils.USE_ACL16_WALLET_CRYPTO:
        ...
        node = Slip21Node(data=se_acl16.slip21_fido_node())
    else:
        seed = _get_seed_without_passphrase()
