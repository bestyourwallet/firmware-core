...
def find_message_handler_module(msg_type: int) -> str:
    ...
    if not utils.BITCOIN_ONLY:
        ...
        # ethereum ukey
        if msg_type == MessageType.EthereumGetAddressUKey:
            return "apps.ethereum.ukey.get_address"
        if msg_type == MessageType.EthereumGetPublicKeyUKey:
            return "apps.ethereum.ukey.get_public_key"
        if msg_type == MessageType.EthereumSignTxUKey:
            return "apps.ethereum.ukey.sign_tx"
        if msg_type == MessageType.EthereumSignTxEIP1559UKey:
            return "apps.ethereum.ukey.sign_tx_eip1559"
        if msg_type == MessageType.EthereumSignTxEIP7702UKey:
            return "apps.ethereum.ukey.sign_tx_eip7702"
        if msg_type == MessageType.EthereumSignMessageUKey:
            return "apps.ethereum.ukey.sign_message"
        if msg_type == MessageType.EthereumVerifyMessageUKey:
            return "apps.ethereum.ukey.verify_message"
        if msg_type == MessageType.EthereumSignTypedDataUKey:
            return "apps.ethereum.ukey.sign_typed_data"
        if msg_type == MessageType.EthereumSignTypedHashUKey:
            return "apps.ethereum.ukey.sign_typed_data_hash"
        ...
        if msg_type == MessageType.AptosSignSIWAMessage:
            return "apps.aptos.sign_siwa_message"
...
def find_registered_handler(
    iface: WireInterface | None, msg_type: int
) -> Handler | None:
    ...
    try:
        ...
        if __debug__:
            print(f"Found handler module {modname} for message type {msg_type}")
...
def _is_address_derivation_message(msg_type: int) -> bool:
    if utils.BITCOIN_ONLY:
        ...
        return msg_type in (
            ...
            MessageType.EthereumGetAddressUKey,
            ...
            MessageType.EthereumGetPublicKeyUKey,
