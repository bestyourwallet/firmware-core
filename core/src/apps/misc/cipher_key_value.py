...
def compute_cipher_key_value(msg: CipherKeyValue, seckey: bytes) -> bytes:
    ...
    if utils.USE_ACL16_WALLET_CRYPTO:
        ...
        if msg.iv and len(msg.iv) == 16:
            iv = msg.iv
            if msg.encrypt:
                return se_acl16.aes256_encrypt(data, msg.value, msg.iv)
            else:
                return se_acl16.aes256_decrypt(data, msg.value, msg.iv)
        else:
            if msg.encrypt:
                return se_acl16.aes256_encrypt(data, msg.value, None)
            else:
                return se_acl16.aes256_decrypt(data, msg.value, None)
