...
from apps.common.pin_constants import AttachCommon, PinType
...
def get_vendor():
    return "trezor.io" if storage.device.is_trezor_compatible() else "ukey.io"


FW_VENDOR_BTC_ONLY = "UKey Bitcoin-only"
...
def get_features() -> Features:
    ...
    from trezor.enums import Capability, UKeyDeviceType, UKeySeType
    ...
    f = Features(
        ...
        ukey_version=utils.UKEY_VERSION,
        ...
        ukey_device_type=UKeyDeviceType.PRO,
        ukey_se_type=UKeySeType.ACL16,
        ukey_board_version=utils.board_version(),
        ukey_boot_version=utils.boot_version(),
        ukey_se01_version=storage.device.get_se01_version(),
        ukey_se02_version=storage.device.get_se02_version(),
        ukey_se03_version=storage.device.get_se03_version(),
        ukey_se04_version=storage.device.get_se04_version(),
        ukey_firmware_version=utils.UKEY_VERSION,
        ukey_serial_no=storage_serial_no,
        ukey_ble_name=uart.get_ble_name(),
        ukey_ble_version=uart.get_ble_version(),
    )
...
def get_ukey_features() -> UkeyFeatures:
    from trezor.enums import UKeyDeviceType, UKeySeType
    from trezor.messages import UkeyFeatures
    from trezor import uart
    ...
    f = UkeyFeatures(
        ...
        ukey_se_type=UKeySeType.ACL16,
        ukey_board_version=utils.board_version(),
        ukey_board_hash=utils.board_hash(),
        ukey_board_build_id=utils.board_build_id(),
        ukey_boot_version=utils.boot_version(),
        ukey_boot_hash=utils.boot_hash(),
        ukey_boot_build_id=utils.boot_build_id(),
        ukey_firmware_version=utils.UKEY_VERSION,
        ukey_firmware_build_id=utils.BUILD_ID[-7:].decode(),
        ukey_firmware_hash=utils.ukey_firmware_hash(),
        ukey_ble_name=uart.get_ble_name(),
        ukey_ble_version=uart.get_ble_version(),
        ukey_ble_build_id=uart.get_ble_build_id(),
        ukey_ble_hash=uart.get_ble_hash(),
        ukey_se01_version=storage.device.get_se01_version(),
        ukey_se01_hash=storage.device.get_se01_hash(),
        ukey_se01_build_id=storage.device.get_se01_build_id(),
        ukey_se01_boot_version=storage.device.get_se01_boot_version(),
        ukey_se01_boot_hash=storage.device.get_se01_boot_hash(),
        ukey_se01_boot_build_id=storage.device.get_se01_boot_build_id(),
        ukey_se02_version=storage.device.get_se02_version(),
        ukey_se02_hash=storage.device.get_se02_hash(),
        ukey_se02_build_id=storage.device.get_se02_build_id(),
        ukey_se02_boot_version=storage.device.get_se02_boot_version(),
        ukey_se02_boot_hash=storage.device.get_se02_boot_hash(),
        ukey_se02_boot_build_id=storage.device.get_se02_boot_build_id(),
        ukey_se03_version=storage.device.get_se03_version(),
        ukey_se03_hash=storage.device.get_se03_hash(),
        ukey_se03_build_id=storage.device.get_se03_build_id(),
        ukey_se03_boot_version=storage.device.get_se03_boot_version(),
        ukey_se03_boot_hash=storage.device.get_se03_boot_hash(),
        ukey_se03_boot_build_id=storage.device.get_se03_boot_build_id(),
        ukey_se04_version=storage.device.get_se04_version(),
        ukey_se04_hash=storage.device.get_se04_hash(),
        ukey_se04_build_id=storage.device.get_se04_build_id(),
        ukey_se04_boot_version=storage.device.get_se04_boot_version(),
        ukey_se04_boot_hash=storage.device.get_se04_boot_hash(),
        ukey_se04_boot_build_id=storage.device.get_se04_boot_build_id(),
    )
    return f
...
async def handle_Initialize(
    ...
    valid_passphrase_state = False
    if passphrase_state:
        if utils.USE_ACL16_WALLET_CRYPTO:
            valid_passphrase_state = se_acl16.check_passphrase_btc_test_address(
                passphrase_state
            )
        else:
            from apps.common.coininfo import by_name
            from apps.bitcoin.verify_message import address_to_script_type

            try:
                address_to_script_type(passphrase_state, by_name("Testnet"))
                valid_passphrase_state = True
            except Exception:
                valid_passphrase_state = False

    if valid_passphrase_state:
        if not utils.USE_ACL16_WALLET_CRYPTO:
            session_id = storage.cache.get_session_id()
        else:
            session_id = se_acl16.get_session_current_id()
    ...
    if not utils.BITCOIN_ONLY:
        ...
        if utils.USE_ACL16_WALLET_CRYPTO:
            if msg.derive_cardano is not None and msg.derive_cardano:
                state = se_acl16.get_session_state()
            ...
            storage.cache.set(storage.cache.APP_COMMON_DERIVE_CARDANO, b"\x01")
...
async def handle_UkeyGetFeatures(
    ctx: wire.Context, msg: UkeyGetFeatures
) -> UkeyFeatures:
    return get_ukey_features()
...
def set_homescreen(show_app_guide: bool = False, into_optscreen: bool = False) -> None:
    ...
    if storage.device.is_initialized():
        ...
        if not device_is_unlocked():
            ...
            if into_optscreen:
                screen.change_state(2)
            else:
                screen.change_state(0)
            if not first_unlock:
                first_unlock = True
                if (
...
async def unlock_device(
    ...
    if not config.is_unlocked():
        await verify_user_pin(
            ...
            fingerprint_subtitle=fingerprint_subtitle,
        )
    else:
    ...
    set_homescreen(into_optscreen=into_optscreen)
...
def boot() -> None:
    ...
    workflow_handlers.register(MessageType.UkeyGetFeatures, handle_UkeyGetFeatures)
