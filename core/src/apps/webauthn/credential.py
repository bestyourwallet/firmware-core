...
from trezor.crypto import bip32, chacha20poly1305, der, hashlib, hmac, random, se_acl16
...
class Credential:
    ...
    def _u2f_sign(self, data: Iterable[bytes]) -> bytes:
        ...
        if utils.USE_ACL16_WALLET_CRYPTO:
            sig = se_acl16.fido_sign_digest(dig.digest())
    ...
    def public_key(self) -> bytes:
        if self.curve == common.COSE_CURVE_P256:
            if utils.USE_ACL16_WALLET_CRYPTO:
                ...
                node = seed.derive_fido_node_with_se(path, _CURVE_NAME[self.curve])
                pubkey = se_acl16.uncompress_pubkey(
...
class U2fCredential(Credential):
    ...
    def public_key(self) -> bytes:
        if utils.USE_ACL16_WALLET_CRYPTO:
            if self.node is None:
                return b""
            return se_acl16.uncompress_pubkey("nist256p1", self.node.public_key())
        else:
    ...
    def generate_key_handle(self) -> None:
        if utils.USE_ACL16_WALLET_CRYPTO:
            self.id, self.node = se_acl16.u2f_gen_handle_and_node(self.rp_id_hash)
        else:
    ...
    def _node_from_handle_by_se(
        ...
        if len(key_handle) != _KEY_HANDLE_LENGTH:
            raise ValueError  # key length mismatch
        try:
            if se_acl16.fido_u2f_validate(rp_id_hash, key_handle):
                cred = U2fCredential()
                cred.id = key_handle
                cred.rp_id_hash = rp_id_hash
