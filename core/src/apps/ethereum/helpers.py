...
if TYPE_CHECKING:
    from trezor.messages import EthereumFieldType, EthereumFieldTypeUKey
...
def get_type_name_ukey(field: EthereumFieldTypeUKey) -> str:
    """Create a string from type definition (like uint256 or bytes16)."""
    from trezor.enums import EthereumDataTypeUKey

    data_type = field.data_type
    size = field.size

    TYPE_TRANSLATION_DICT = {
        EthereumDataTypeUKey.UINT: "uint",
        EthereumDataTypeUKey.INT: "int",
        EthereumDataTypeUKey.BYTES: "bytes",
        EthereumDataTypeUKey.STRING: "string",
        EthereumDataTypeUKey.BOOL: "bool",
        EthereumDataTypeUKey.ADDRESS: "address",
    }

    if data_type == EthereumDataTypeUKey.STRUCT:
        assert field.struct_name is not None  # validate_field_type
        return field.struct_name
    elif data_type == EthereumDataTypeUKey.ARRAY:
        assert field.entry_type is not None  # validate_field_type
        type_name = get_type_name_ukey(field.entry_type)
        if size is None:
            return f"{type_name}[]"
        else:
            return f"{type_name}[{size}]"
    elif data_type in (EthereumDataTypeUKey.UINT, EthereumDataTypeUKey.INT):
        assert size is not None  # validate_field_type
        return TYPE_TRANSLATION_DICT[data_type] + str(size * 8)
    elif data_type == EthereumDataTypeUKey.BYTES:
        if size:
            return TYPE_TRANSLATION_DICT[data_type] + str(size)
        else:
            return TYPE_TRANSLATION_DICT[data_type]
...
def get_color_and_icon(chain_id: int | None):
    ...
    if chain_id is None:
        return lv.color_hex(0x255770), theme_path_default("chain_evm_eth.png")
    network: EthereumNetworkInfo | None = networks.by_chain_id(chain_id)
    if network and network is not networks.UNKNOWN_NETWORK:
        return lv.color_hex(network.primary_color), theme_path_default(network.icon)
    else:
        return lv.color_hex(0x255770), theme_path_default(
            "chain_evm_eth.png"
        )  # evm-none
