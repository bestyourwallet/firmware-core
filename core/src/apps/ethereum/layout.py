...
from trezor.enums import ButtonRequestType, EthereumDataType, EthereumDataTypeUKey
from trezor.lvglui.i18n import gettext as _, keys as i18n_keys
from trezor.lvglui.scrs import lv_theme, theme_path_default, theme_path_png
from trezor.messages import (
    EthereumFieldType,
    EthereumFieldTypeUKey,
...
async def show_invalid_delegate(ctx: Context) -> None:
    ...
    await show_warning(
        ...
        btn_yes_bg_color=lv_theme.APP_WARNING_BG,
        btn_yes_text_color=lv_theme.BTN_YES_FG,
    )
...
def require_show_approve_overview(
    ...
    return should_show_approve_details(
        ...
        provider_icon_path=provider_icon_path
        or theme_path_default("chain_provider-default.png"),
        title=title,
...
async def should_show_struct(
    ctx: Context,
    description: str,
    data_members: list[EthereumStructMember] | list[EthereumStructMemberUKey],
...
async def confirm_typed_value_ukey(
    ctx: Context,
    name: str,
    value: bytes,
    parent_objects: list[str],
    field: EthereumFieldTypeUKey,
    array_index: int | None = None,
) -> None:
    type_name = get_type_name_ukey(field)
    ...
    if field.data_type in (
        EthereumDataTypeUKey.ADDRESS,
        EthereumDataTypeUKey.BYTES,
    ):
        await confirm_blob(
            ctx,
            "confirm_typed_value",
