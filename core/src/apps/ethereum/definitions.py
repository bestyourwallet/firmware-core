...
class Definitions:
    ...
    def from_encoded(
        ...
        if encoded_network is not None and chain_id is None:
            magic = encoded_network[0:5]
            tn = decode_definition(encoded_network, EthereumNetworkInfo)
            chain_id = tn.chain_id
            slip44 = None
            # ukey chain_id
            if magic != b"trzd1":
                chain_id = int.from_bytes(encoded_network, "big")
        ...
        if chain_id is not None:
            ...
            return cls(UNKNOWN_NETWORK, {}, chain_id)
        ...
        return cls(network, tokens, chain_id)
