...
from trezor.enums import EthereumDataTypeUKey as EthereumDataType
from trezor.lvglui.i18n import gettext as _, keys as i18n_keys
from trezor.messages import (
    EthereumFieldTypeUKey as EthereumFieldType,
    EthereumGnosisSafeTxAck,
    EthereumGnosisSafeTxRequest,
    EthereumSignTypedDataUKey as EthereumSignTypedData,
    EthereumTypedDataSignatureUKey as EthereumTypedDataSignature,
    EthereumTypedDataStructAckUKey as EthereumTypedDataStructAck,
    EthereumTypedDataStructRequestUKey as EthereumTypedDataStructRequest,
    EthereumTypedDataValueAckUKey as EthereumTypedDataValueAck,
    EthereumTypedDataValueRequestUKey as EthereumTypedDataValueRequest,
)
...
class TypedDataEnvelope:
    ...
    def encode_type(self, primary_type: str) -> bytes:
        ...
        for type_name in [primary_type] + sorted(deps):
            ...
            fields = ",".join(f"{get_type_name_ukey(m.type)} {m.name}" for m in members)
            ...
            if field_type.data_type == EthereumDataType.STRUCT:
                ...
                if show_data:
                    show_array = await should_show_array(
                        ctx=self.ctx,
                        parent_objects=current_parent_objects,
                        data_type=get_type_name_ukey(entry_type),
                        size=array_size,
                    )
                else:
                    show_array = False
                ...
                for i in range(array_size):
                    ...
                    if entry_type.data_type == EthereumDataType.STRUCT:
                        ...
                        if show_array:
                            await confirm_typed_value_ukey(
                                ctx=self.ctx,
                                name=field_name,
                                value=value,
                                parent_objects=parent_objects,
