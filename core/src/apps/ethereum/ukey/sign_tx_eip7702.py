...
from trezor.messages import (
    ...
    EthereumAuthorizationUKey as EthereumAuthorizationUKey,
    EthereumTxRequestUKey as EthereumTxRequest,
)
...
if TYPE_CHECKING:
    from trezor.messages import EthereumSignTxEIP7702UKey as EthereumSignTxEIP7702
...
def authorization_list_item_length(item: EthereumAuthorizationUKey) -> int:
...
def authorization_list_length(
    authorization_list: list[EthereumAuthorizationUKey],
) -> int:
    payload_length = sum(
...
def write_authorization_list(
    w: HashWriter, authorization_list: list[EthereumAuthorizationUKey]
) -> None:
    payload_length = sum(
        authorization_list_item_length(item) for item in authorization_list
    )
...
def make_authorization_digest(msg: EthereumAuthorizationUKey) -> bytes:
