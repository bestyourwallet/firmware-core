...
async def show_ti_confirmation_failure(ctx: wire.GenericContext) -> None:
...
async def show_dry_run_result(
    ...
    if result:
        ...
        if not is_slip39:
            if utils.get_current_backup_type() in (
                utils.BACKUP_METHOD_LITE,
                utils.BACKUP_METHOD_RING,
            ):
                await backup_with_lite(ctx, mnemonics, recovery_check=True)
            elif utils.get_current_backup_type() == utils.BACKUP_METHOD_SEED_TI:
                await backup_with_seed_ti(ctx, mnemonics, recovery_check=True)
        else:
...
async def show_share_already_added(ctx: wire.GenericContext) -> None:
    ...
    await show_warning(
        ...
        btn_yes_bg_color=lv_theme.APP_INFO_BG,
    )
