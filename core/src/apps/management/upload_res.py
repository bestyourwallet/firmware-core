...
async def upload_res(ctx: wire.Context, msg: ResourceUpload) -> Success:
    ...
    if res_type == ResourceType.WallPaper:
        ...
        file_full_path = f"1:/res/nfts/imgs/{file_name}.jpg"
        zoom_path = f"1:/res/nfts/zooms/zoom-{file_name}.png"
    ...
    try:
        ...
        if replace:
            ...
            if res_type == ResourceType.WallPaper:
                ...
                noext_name = zoom_file[: -(len(res_ext) + 1)]
                io.fatfs.unlink(f"1:/res/nfts/zooms/{noext_name}.png")
                noext_name = file_name[: -(len(res_ext) + 1)]
                io.fatfs.unlink(f"1:/res/nfts/imgs/{noext_name}.jpg")
                io.fatfs.unlink(f"1:/res/nfts/desc/{noext_name}.json")
