...
RESOURCE_DIR = "/res"
RESOURCE_LIGHT_DIR = RESOURCE_DIR + "/light"
RESOURCE_DARK_DIR = RESOURCE_DIR + "/dark"
...
async def update_res(ctx: wire.Context, msg: ResourceUpdate) -> Success:
    ...
    try:
        if not is_update_boot:
            ensure_resource_dirs(file_path)
...
def ensure_resource_dirs(file_path: str) -> None:
    io.fatfs.mkdir(RESOURCE_DIR, True)
    parent_path = file_path[: file_path.rindex("/")]
    if parent_path == RESOURCE_DIR:
        return
    ...
    for part in parent_path.split("/"):
        if not part:
            continue
        current_path += "/" + part
        io.fatfs.mkdir(current_path, True)


def make_file_path(file_name: str, update_boot: bool = False) -> str:
    if update_boot:
        return f"/boot/{file_name}"
    if not file_name or file_name == "." or file_name == "..":
        raise wire.DataError("Invalid resource path")
    if "/" in file_name:
        return make_resource_file_path(file_name)
    if file_name.startswith("li_"):
        return f"{RESOURCE_LIGHT_DIR}/{file_name}"
    if file_name.startswith("da_"):
        return f"{RESOURCE_DARK_DIR}/{file_name}"
    return f"{RESOURCE_DIR}/{file_name}"


def make_resource_file_path(file_name: str) -> str:
    if not file_name:
        raise wire.DataError("File name required")
    if file_name.startswith("/"):
        if not file_name.startswith(RESOURCE_DIR + "/"):
            raise wire.DataError("Invalid resource path")
        file_path = file_name
    else:
        file_path = f"{RESOURCE_DIR}/{file_name}"

    for index, part in enumerate(file_path.split("/")):
        if index == 0:
            continue
