...
from trezor.ui.layouts import confirm_backup, confirm_reset_device, show_ukey_app_guide
...
def _is_valid_backup_type(backup_type: int) -> TypeGuard[BackupType]:
    return backup_type in (B39, S39_B, S39_A, S39_SE, S39_BE, S39_AE)
        ...
        if perform_backup:
            ...
            if result is not None and isinstance(result, tuple):
                if result == (0, 0):
                    ...
                    continue
                selected_backup_type, selected_strength = result
                if not _is_valid_backup_type(selected_backup_type):
                    raise RuntimeError("Unknown backup type.")
                selection_changed = (
                    selected_backup_type != backup_type or selected_strength != strength
                )
                if selection_changed:
                    ...
                    if msg.display_random:
                        await layout.show_internal_entropy(ctx, int_entropy)
                backup_type, strength = selected_backup_type, selected_strength
                if selection_changed:
                    skip_backup_warning = False
                continue
            await layout.show_create_success(ctx)
            break
        else:
            break
