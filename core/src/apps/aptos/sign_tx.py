...
from .transaction import AptosDecodeError, Transaction
...
async def sign_tx(
    ...
    tx_type = int(msg.tx_type) if msg.tx_type is not None else 0
    try:
        tx = Transaction.deserialize(msg.raw_tx, tx_type)
    except AptosDecodeError:
        await confirm_blind_sign_common(ctx, address, msg.raw_tx)
    else:
        await tx.layout(ctx, address)
    prefix_bytes = sha3_256(aptos_transaction_prefix(tx_type)).digest()
