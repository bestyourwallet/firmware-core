...
class Popup(lv.obj):
    def __init__(
        ...
        self.set_pos(0, 40)
        self.add_style(
            StyleWrapper().pad_all(0).bg_opa(255).bg_color(lv_theme.DT_BG)
            # .bg_opa(lv.OPA.TRANSP)
            .border_width(0),
            0,
        )
        self.close_img = lv.img(self)
        self.close_img.set_src(theme_path_png("nav-back"))
        self.close_img.align(lv.ALIGN.TOP_LEFT, 15, 0)
        ...
        self.title = Title(self, self.icon, title, (0, 36))
