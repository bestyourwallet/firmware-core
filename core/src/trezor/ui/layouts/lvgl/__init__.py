...
if TYPE_CHECKING:
    from typing import Any, Awaitable, Iterable, NoReturn, Sequence, TypeAlias

    PropertyType: TypeAlias = tuple[str | None, str | bytes | None]
    ExceptionType: TypeAlias = type[BaseException] | BaseException
...
__all__ = (
    ...
    "confirm_aptos_transfer",
    "confirm_sui_transfer",
    "should_show_details",
...
async def confirm_action(
    ...
    if icon is None:
        icon = theme_path_png("triangle-warning")
    elif icon.strip() == "":
        icon = None
    ...
    # confirm_screen.content_area.set_style_min_height(760, 0)
    # if hasattr(confirm_screen, "bnt_container") and confirm_screen.bnt_container:
    #     confirm_screen.bnt_container.align_to(confirm_screen.content_area, lv.ALIGN.OUT_BOTTOM_MID, 0, -190)
    confirm_screen.update_btn_layout()
...
async def confirm_reset_device(
    ...
    # restscreen.content_area.set_style_min_height(760, 0)
    # if hasattr(restscreen, "bnt_container") and restscreen.bnt_container:
    #     restscreen.bnt_container.align_to(restscreen.content_area, lv.ALIGN.OUT_BOTTOM_MID, 0, -190)
    # restscreen.update_btn_layout()
    await raise_if_cancelled(
        interact(
            ctx,
            restscreen,
...
async def confirm_backup(ctx: wire.GenericContext) -> bool:
    ...
    confirm_text = _(i18n_keys.BUTTON__CONTINUE)
    cancel_text = _(i18n_keys.BUTTON__DONT_PROTECT)
    # icon = "A:/res/success.png"
    icon = theme_path_png("hidden-wallet")
    if ctx == wire.DUMMY_CONTEXT:
        cancel_text = ""
    screen = FullSizeWindow(
        title, subtitle, confirm_text, cancel_text, icon_path=icon, nav_back=False
    )
    # screen.content_area.set_style_min_height(760, 0)
    # if hasattr(screen, "bnt_container") and screen.bnt_container:
    #     screen.bnt_container.align_to(screen.content_area, lv.ALIGN.OUT_BOTTOM_MID, 0, -190)
...
async def confirm_path_warning(
    ...
    screen.btn_yes.enable()  # bg_color=lv_colors.UKEY_O_YELLOW, text_color=lv_colors.BLACK)
    # screen.content_area.set_style_min_height(760, 0)
    # if hasattr(screen, "bnt_container") and screen.bnt_container:
    #     screen.bnt_container.align_to(screen.content_area, lv.ALIGN.OUT_BOTTOM_MID, 0, -190)
    screen.update_btn_layout()
...
async def _show_modal(
    ...
    if btn_yes_bg_color:
        ...
        #     bg_color=btn_yes_bg_color or lv_colors.UKEY_O_GREEN,
...
def show_warning(
    ...
    icon: str = theme_path_png("warning"),
...
def show_success(
    ...
    return _show_modal(
        ...
        button_cancel=None,
        # icon="A:/res/success.png",
        icon=theme_path_png("success") if icon is None else icon,
        icon_color=ui.GREEN,
        nav_back=nav_back,
    )
...
async def show_lite_card_exit(
    ...
    screen.btn_yes.enable()  # bg_color=lv_colors.UKEY_O_RED_1, text_color=lv_colors.BLACK)
    await raise_if_cancelled(
        interact(
            ctx,
            screen,
...
async def should_show_details_eip7702(
    ...
    res = await interact(
        ...
        TransactionOverview(
            ...
            card_icon=theme_path_png("group-icon-more"),
            items=(
...
async def confirm_blob(
    ...
    icon: str | None = theme_path_png("warning"),
...
def draw_simple_text(
    ...
    icon_path: str | None = theme_path_png("warning"),
...
async def request_pin_on_device(
    ...
    if attempts_remaining is None or attempts_remaining == device.PIN_MAX_ATTEMPTS:
        ...
        if standy_wall_only and passphrase.is_passphrase_pin_enabled():
            ...
            subprompt = fingerprint_subtitle
...
async def show_ukey_app_guide():
    # if not __debug__:
    from trezor.lvglui.scrs import app_guide

    app_guide.GuideAppDownload(nav_back=False)
...
async def confirm_set_homescreen(ctx, replace: bool = False):
    await confirm_action(
        ...
        icon=theme_path_png("card-voided"),
    )
...
async def confirm_del_wallpaper(ctx, confirm_callback):
    ...
    #     bg_color=lv_colors.UKEY_O_RED_1, text_color=lv_colors.BLACK
    # )
    # confirm_screen.content_area.set_style_min_height(760, 0)
    # if hasattr(confirm_screen, "bnt_container") and confirm_screen.bnt_container:
    #     confirm_screen.bnt_container.align_to(confirm_screen.content_area, lv.ALIGN.OUT_BOTTOM_MID, 0, -190)
...
async def confirm_exit_backup_process(ctx: wire.GenericContext) -> None:
    from trezor.lvglui.scrs.template import Modal

    screen = Modal(
        _(i18n_keys.TITLE__EXIT_BACKUP_PROCESS),
        _(i18n_keys.TITLE__EXIT_BACKUP_PROCESS_DESC),
        confirm_text=_(i18n_keys.BUTTON__EXIT),
        cancel_text=_(i18n_keys.BUTTON__CANCEL),
        icon_path=None,
    )
    result = await ctx.wait(screen.request())
...
# backup_with_keytag
async def backup_with_seed_ti(
    ctx: wire.GenericContext, mnemonics: bytes, recovery_check: bool = False
) -> None:
    from apps.management import serial_codec
    from trezor.lvglui.scrs.common import FullSizeWindow
    from trezor.lvglui.scrs.homescreen import BackupWallet

    # import random
    from trezor.crypto import bip39, random

    mnemonics_str = mnemonics.decode() if isinstance(mnemonics, bytes) else mnemonics
    words = mnemonics_str.split()
    mnemonics_ints = [bip39.find(word) for word in words]
    while True:
        ...
        #     icon_path=theme_path_default("serial-security-protection.png"),
        #     anim_dir=0,
        #     button_layout=0,
        # )
        # ask_screen.subtitle.set_style_text_align(lv.TEXT_ALIGN.LEFT, 0)
        ...
        while True:
            exit_loop = False
            from trezor.lvglui.scrs.pinscreen import InputTiSerial
            from trezor.lvglui.scrs.recovery_device import ShowTiNumber, CheckTiSeedTips
            from trezor.lvglui.scrs.pinscreen import InputTiCodeConfirm

            display_mnemonics_ints = mnemonics_ints
            if ask_result == 1:
                while True:
                    screen = InputTiSerial()
                    serial = await ctx.wait(screen.request())
                    if not serial:
                        if await confirm_exit_backup_process(ctx):
                            exit_loop = True
                            break
                    ...
                    try:
                        ...
                        display_mnemonics_ints = (
                            serial_codec.encode_ti_mnemonic_indexes(
                                mnemonics_ints, permutation
                            )
                        )
                    except ValueError:
                        await show_warning(
                            ...
                            header=_(i18n_keys.TITLE__TI_RECOVER_INCORRECT),
                            subheader="",
                            icon=theme_path_png("danger"),
                            content=_(i18n_keys.SUBTITLE__TI_RECOVER_INCORRECT),
            ...
            if exit_loop:
                break
            show_screen = ShowTiNumber()
            for i, k in enumerate(display_mnemonics_ints):
                while True:
                    show_screen.set_number(i + 1, k)
                    screen_status = not await ctx.wait(show_screen.request())
                    if screen_status:
                        if await confirm_exit_backup_process(ctx):
                            exit_loop = True
                            break
            ...
            if exit_loop:
                break
            show_screen.load_screen(BackupWallet(), destroy_self=True)

            while True:
                check_screen = await ctx.wait(CheckTiSeedTips().request())
                if check_screen == 0:
                    if await confirm_exit_backup_process(ctx):
                        exit_loop = True
                        break
            ...
            idxs = list(range(len(display_mnemonics_ints)))
            random.shuffle(idxs)
            idxs = idxs[:3]
            idxs.sort()
            for i in idxs:
                while True:
                    ti_confirm = InputTiCodeConfirm(BackupWallet(), i + 1)
                    ...
                    if int(data) != display_mnemonics_ints[i]:
                        exit_loop = True
                        error_confirm = FullSizeWindow(
                            _(i18n_keys.TITLE__TI_INCORRECT_NUMBER),
                            _(i18n_keys.SUBTITLE__TI_DEVICE_BACKUP_INCORRECT_NUMBER),
                            cancel_text=_(i18n_keys.BUTTON__TRY_AGAIN),
                            icon_path=theme_path_png("danger"),
                            anim_dir=0,
                            nav_back=False,
                        )
                        result = await ctx.wait(error_confirm.request())
                        if result:
                            exit_loop = True
                            break
                    break
            ...
            final_confirm = FullSizeWindow(
                ...
                _(i18n_keys.SUBTITLE__TI_BACKUP_COMPLETED),
                confirm_text=_(i18n_keys.BUTTON__CONTINUE),
                # cancel_text=_(i18n_keys.BUTTON__CANCEL),
                icon_path=theme_path_png(
                    "tools/success"
                ),  # "A:/res/icon-tips-blue.png",
                anim_dir=0,
                nav_back=False,
            )
            result = await ctx.wait(final_confirm.request())
            if result:
                exit_loop = True
                break
            break
...
async def confirm_aptos_transfer(
    ctx: wire.GenericContext,
    sender: str,
    recipient: str,
    amount: str,
    ...
    secondary_signers: str | None = None,
) -> None:
    from trezor.lvglui.scrs.template import AptosTransfer
    from trezor.strings import strip_amount

    striped_amount, striped = strip_amount(amount)
    title = _(i18n_keys.TITLE__SEND_MULTILINE).format(striped_amount)
    if await should_show_details_new(
        ctx,
        title=title,
        br_code=ButtonRequestType.SignTx,
        ...
        await raise_if_cancelled(
            interact(ctx, screen, "confirm_aptos_transfer", ButtonRequestType.SignTx)
        )


async def confirm_sui_transfer(
    ctx: wire.GenericContext,
    sender: str,
    recipient: str,
    amount: str,
    ...
    from trezor.lvglui.scrs.template import SuiTransfer
    from trezor.strings import strip_amount
    ...
    if await should_show_details_new(
        ...
        screen = SuiTransfer(
            ...
            gas_budget,
            signer,
            tx_type or _(i18n_keys.LIST_VALUE__SUI_TRANSFER),
            token,
            ctx.primary_color,
            ctx.icon_path,
            striped=striped,
        ...
        await raise_if_cancelled(
            interact(ctx, screen, "confirm_sui_transfer", ButtonRequestType.SignTx)
        )
