...
async def request_word_count(
    ...
    counts = (12, 18, 20, 24, 33)
    if dry_run:
        title = _(i18n_keys.TITLE__RECOVERY_PHRASE)
    ...
    if allowed_counts is not None:
        counts = tuple(count for count in counts if count in allowed_counts)
        if not counts:
            raise RuntimeError

    optional_str = "\n".join(
        _(i18n_keys.OPTION__STR_WRODS).format(count) for count in counts
    )
...
async def continue_recovery(
    ...
    # screen.bnt_container.align(lv.ALIGN.BOTTOM_MID, 0, 0)
