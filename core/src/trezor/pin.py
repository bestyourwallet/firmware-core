...
def show_pin_timeout(_seconds: int, progress: int, message: str) -> bool:
    from trezor.lvglui.scrs import lv, lv_theme
    ...
    if _scr_need_create:
        ...
        arc_loader.set_style_arc_color(lv_theme.PROCESS_BAR_FG, lv.PART.INDICATOR)
        arc_loader.set_style_bg_color(lv_theme.PROCESS_BAR_BG, 0)
        arc_loader.center()

        _scr.arc_loader = arc_loader

        _timer = lv.timer_create(_timer_callback, 10000, None)
        # import time
        # time.sleep(100)
