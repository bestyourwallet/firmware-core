...
class QRTask:
    ...
    def is_camera_paused(self) -> bool:
        return self.pause_count > 0

    def pause_camera(self) -> None:
        self.pause_count += 1
        if __debug__ and self.scanning:
            print("qr camera pause", self.pause_count)

    def resume_camera(self) -> None:
        if self.pause_count > 0:
            self.pause_count -= 1
            if __debug__ and self.scanning:
                print("qr camera resume", self.pause_count)
...
def ti_scan_qr(callback_obj):
    async def camear_scan():
        ...
        while True:
            ...
            if qr_data:
                if __debug__:
                    print(f"ti_scan_qr camear_scan: {qr_data}")  # .decode())
                # Call on_qr_scanned method if it exists
                if hasattr(callback_obj, "on_qr_scanned"):
                    camera.stop()
                    # Stop the camera before waking the parent screen so LVGL redraws
                    # do not race with the direct camera overlay refresh.
                    await callback_obj.on_qr_scanned(qr_data.decode())
...
def pause_camera_refresh() -> None:
    qr_task.pause_camera()


def resume_camera_refresh() -> None:
    qr_task.resume_camera()
