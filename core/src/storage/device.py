...
BLE_NAME_MAXLENGTH = const(20)
BLE_VERSION_MAXLENGTH = const(8)
PREVIOUS_LABEL_MAXLENGTH = const(13)
LAYOUT_MAXLENGTH = const(16)

SE_COMPONENT_01 = const(0)
SE_COMPONENT_02 = const(1)
SE_COMPONENT_03 = const(2)
SE_COMPONENT_04 = const(3)
...
_THEME_VALUE: int | None = None
_LAYOUT_VALUE: str | None = None
...
_SE_COMPONENT_INFO_VALUE: list[tuple[str, str, bytes, str, str, bytes] | None] = [
    None,
    None,
    None,
    None,
...
_LOCKSCREEN_SHOW_MODEL_NAME_VALUE: bool | None = None
_LOCKSCREEN_SHOW_BLE_ID_VALUE: bool | None = None

# ACL16 storage exposes fixed public/private regions, so keys are byte offsets.
if utils.USE_ACL16:
    import uctypes
    ...
    def _add_pad(desc: uctypes.StructDict, used_size: int) -> uctypes.StructDict:
        pad_len = (-used_size) & 3
        if pad_len:
            desc["_pad"] = (used_size | uctypes.ARRAY, pad_len | uctypes.UINT8)
        return desc

    struct_bool: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "value": 4 | uctypes.UINT8,
        },
        _RECORD_HEADER_SIZE + 1,
    )

    struct_uint32: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "value": 4 | uctypes.UINT32,
        },
        _RECORD_HEADER_SIZE + 4,
    )

    struct_uuid: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "uuid": (4 | uctypes.ARRAY, 25 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + 25,
    )

    struct_language: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "language": (4 | uctypes.ARRAY, LANGUAGE_MAXLENGTH + 1 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + LANGUAGE_MAXLENGTH + 1,
    )

    struct_label_deprecated: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "uuid": (4 | uctypes.ARRAY, 13 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + 13,
    )

    struct_label: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "uuid": (4 | uctypes.ARRAY, LABEL_MAXLENGTH + 1 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + LABEL_MAXLENGTH + 1,
    )

    struct_homescreen: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "uuid": (4 | uctypes.ARRAY, HOMESCREEN_PATH_MAXSIZE | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + HOMESCREEN_PATH_MAXSIZE,
    )

    struct_sessionkey: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "uuid": (4 | uctypes.ARRAY, 16 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + 16,
    )

    struct_ble_name: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "ble_name": (4 | uctypes.ARRAY, BLE_NAME_MAXLENGTH | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + BLE_NAME_MAXLENGTH,
    )

    struct_ble_version: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "ble_version": (4 | uctypes.ARRAY, BLE_VERSION_MAXLENGTH | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + BLE_VERSION_MAXLENGTH,
    )

    struct_layout: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "layout": (4 | uctypes.ARRAY, LAYOUT_MAXLENGTH | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + LAYOUT_MAXLENGTH,
    )

    struct_SD_auth_key: uctypes.StructDict = _add_pad(
        {
            "magic": 0 | uctypes.UINT8,
            "data_len": 1 | uctypes.UINT8,
            "crc16": 2 | uctypes.UINT16,
            "ble_version": (4 | uctypes.ARRAY, 16 | uctypes.UINT8),
        },
        _RECORD_HEADER_SIZE + 16,
    )
    ...
    offset += uctypes.sizeof(struct_uint32, uctypes.LITTLE_ENDIAN)
    struct_public["theme"] = (offset, struct_uint32)
    offset += uctypes.sizeof(struct_uint32, uctypes.LITTLE_ENDIAN)
    struct_public["layout"] = (offset, struct_layout)
    offset += uctypes.sizeof(struct_layout, uctypes.LITTLE_ENDIAN)
    ...
    offset += uctypes.sizeof(struct_bool, uctypes.LITTLE_ENDIAN)
    struct_public["lockscreen_show_model_name"] = (offset, struct_bool)
    offset += uctypes.sizeof(struct_bool, uctypes.LITTLE_ENDIAN)
    struct_public["lockscreen_show_ble_id"] = (offset, struct_bool)
    offset += uctypes.sizeof(struct_bool, uctypes.LITTLE_ENDIAN)
    ...
    def get_storage_record_size(key: int) -> int:
        table = struct_private if key & _PRIVATE_FLAG else struct_public
        offset = key & ~_PRIVATE_FLAG
        for record_offset, record_struct in table.values():
            if record_offset == offset:
                return uctypes.sizeof(record_struct, uctypes.LITTLE_ENDIAN)
        raise ValueError
    ...
    _LOCKSCREEN_SHOW_MODEL_NAME = struct_public["lockscreen_show_model_name"][0]
    _LOCKSCREEN_SHOW_BLE_ID = struct_public["lockscreen_show_ble_id"][0]
    _THEME = struct_public["theme"][0]
    _LAYOUT = struct_public["layout"][0]
    ...
    _LOCKSCREEN_SHOW_MODEL_NAME = (0x95)  # bool
    _LOCKSCREEN_SHOW_BLE_ID = (0x96)  # bool
    _THEME = (0x93)  # int
    _LAYOUT = (0x94)  # str
    # fmt: on
...
def is_reset_flag_set() -> bool:
    if utils.EMULATOR:
        return False
    return bool(config.is_reset_flag_set())
...
def is_lockscreen_show_model_name() -> bool:
    global _LOCKSCREEN_SHOW_MODEL_NAME_VALUE
    if _LOCKSCREEN_SHOW_MODEL_NAME_VALUE is None:
        value = common.get(_NAMESPACE, _LOCKSCREEN_SHOW_MODEL_NAME, public=True)
        _LOCKSCREEN_SHOW_MODEL_NAME_VALUE = (
            True if value is None else value == common._TRUE_BYTE
        )
    return _LOCKSCREEN_SHOW_MODEL_NAME_VALUE


def set_lockscreen_show_model_name(enable: bool) -> None:
    global _LOCKSCREEN_SHOW_MODEL_NAME_VALUE
    common.set_bool(
        _NAMESPACE,
        _LOCKSCREEN_SHOW_MODEL_NAME,
        enable,
        public=True,
    )
    _LOCKSCREEN_SHOW_MODEL_NAME_VALUE = enable


def is_lockscreen_show_ble_id() -> bool:
    global _LOCKSCREEN_SHOW_BLE_ID_VALUE
    if _LOCKSCREEN_SHOW_BLE_ID_VALUE is None:
        value = common.get(_NAMESPACE, _LOCKSCREEN_SHOW_BLE_ID, public=True)
        _LOCKSCREEN_SHOW_BLE_ID_VALUE = (
            False if value is None else value == common._TRUE_BYTE
        )
    return _LOCKSCREEN_SHOW_BLE_ID_VALUE


def set_lockscreen_show_ble_id(enable: bool) -> None:
    global _LOCKSCREEN_SHOW_BLE_ID_VALUE
    common.set_bool(
        _NAMESPACE,
        _LOCKSCREEN_SHOW_BLE_ID,
        enable,
        public=True,
    )
    _LOCKSCREEN_SHOW_BLE_ID_VALUE = enable
...
def get_label() -> str:
    ...
    Returns:
        str: if label == "", return default label "UKey Core 26" instead
    """
    global _LABEL_VALUE
...
def get_homescreen() -> str | None:
    ...
    if _HOMESCREEN_VALUE is None:
        ...
        _HOMESCREEN_VALUE = (
            homescreen.decode() if homescreen else get_default_wallpaper()
        )
...
def is_trezor_compatible() -> bool:
    ...
    if _TREZOR_COMPATIBLE_VALUE is None:
        ...
        _TREZOR_COMPATIBLE_VALUE = (
            False if enabled is None else enabled == common._TRUE_BYTE
        )
...
def get_se_component_info(
    component_id: int,
) -> tuple[str, str, bytes, str, str, bytes]:
    if component_id < SE_COMPONENT_01 or component_id > SE_COMPONENT_04:
        raise ValueError("invalid SE component id")

    info = _SE_COMPONENT_INFO_VALUE[component_id]
    if info is None:
        info = utils.se_component_info(component_id)
        _SE_COMPONENT_INFO_VALUE[component_id] = info
    return info


def get_se01_version() -> str:
    return get_se_component_info(SE_COMPONENT_01)[0]


def get_se01_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_01)[1]


def get_se01_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_01)[2]


def get_se01_boot_version() -> str:
    return get_se_component_info(SE_COMPONENT_01)[3]


def get_se01_boot_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_01)[4]


def get_se01_boot_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_01)[5]


def get_se02_version() -> str:
    return get_se_component_info(SE_COMPONENT_02)[0]


def get_se02_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_02)[1]


def get_se02_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_02)[2]


def get_se02_boot_version() -> str:
    return get_se_component_info(SE_COMPONENT_02)[3]


def get_se02_boot_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_02)[4]


def get_se02_boot_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_02)[5]


def get_se03_version() -> str:
    return get_se_component_info(SE_COMPONENT_03)[0]


def get_se03_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_03)[1]


def get_se03_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_03)[2]


def get_se03_boot_version() -> str:
    return get_se_component_info(SE_COMPONENT_03)[3]


def get_se03_boot_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_03)[4]


def get_se03_boot_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_03)[5]


def get_se04_version() -> str:
    return get_se_component_info(SE_COMPONENT_04)[0]


def get_se04_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_04)[1]


def get_se04_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_04)[2]


def get_se04_boot_version() -> str:
    return get_se_component_info(SE_COMPONENT_04)[3]


def get_se04_boot_build_id() -> str:
    return get_se_component_info(SE_COMPONENT_04)[4]


def get_se04_boot_hash() -> bytes:
    return get_se_component_info(SE_COMPONENT_04)[5]
...
def set_theme(theme: int) -> None:
    global _THEME_VALUE
    # valid value range: 0 (light), 1 (dark), 2 (auto)
    if theme not in (0, 1, 2):
        raise ValueError("Theme must be 0 (light), 1 (dark), or 2 (auto)")
    common.set(_NAMESPACE, _THEME, theme.to_bytes(1, "big"), public=True)
    _THEME_VALUE = theme


def get_theme() -> int:
    global _THEME_VALUE
    if _THEME_VALUE is None:
        val = common.get(_NAMESPACE, _THEME, public=True)
        if val is None:
            # default theme is dark (1)
            _THEME_VALUE = 1
        else:
            _THEME_VALUE = int.from_bytes(val, "big")
        if _THEME_VALUE not in (0, 1, 2):
            _THEME_VALUE = 1
    return _THEME_VALUE
...
def set_layout(layout: str) -> None:
    ...
    if len(layout.encode("utf-8")) > LAYOUT_MAXLENGTH:
        raise ValueError  # layout too long

    if layout not in ("small", "big"):
        raise ValueError("Layout must be 'small' or 'big'")

    _LAYOUT_VALUE = layout
    common.set(_NAMESPACE, _LAYOUT, layout.encode(), True)  # public


def get_layout() -> str:
    global _LAYOUT_VALUE
    if _LAYOUT_VALUE is None:
        layout = common.get(_NAMESPACE, _LAYOUT, public=True)
        if layout is None:
            # default layout is small
            _LAYOUT_VALUE = "small"
        else:
            _LAYOUT_VALUE = layout.decode()
        if _LAYOUT_VALUE not in ("small", "big"):
            _LAYOUT_VALUE = "small"
...
def clear_global_cache() -> None:
    ...
    global _HAS_PROMPTED_FINGERPRINT_VALUE
    global _FINGER_FAILED_COUNT_VALUE
    global _SE_COMPONENT_INFO_VALUE
    global _DEVICE_ID_VALUE
    global _STORAGE_SIZE_VALUE
    global _SERIAL_NUMBER_VALUE
    global _TREZOR_COMPATIBLE_VALUE
    ...
    _SE_COMPONENT_INFO_VALUE = [None, None, None, None]
