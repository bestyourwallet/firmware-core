...
def start_session(received_session_id: bytes | None = None) -> bytes | None:
    if not utils.USE_ACL16_WALLET_CRYPTO:
        ...
        received_session_id = selected_session.export_session_id()
        _SESSION_ID[:] = received_session_id
        return received_session_id
    else:
        received_session_id = se_acl16.start_session(received_session_id)
...
def is_session_started() -> bool:
    if not utils.USE_ACL16_WALLET_CRYPTO:
        ...
        return se_acl16.session_is_open()
