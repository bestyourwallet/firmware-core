...
#define FATFS_UPDATE_RES_SKIPPED 0
#define FATFS_UPDATE_RES_PENDING 1
#define FATFS_UPDATE_RES_READY 2
#define FATFS_UPDATE_RES_DONE 3

#define FATFS_RES_ROOT_PATH "0:/res"
#define FATFS_RES_LIGHT_PATH "0:/res/light"
#define FATFS_RES_DARK_PATH "0:/res/dark"
#define FATFS_RES_DEFAULT_PATH "0:/res/default"
#define FATFS_RES_RESOURCE_MARKER_NAME ".UKEY_RESOURCE"
#define FATFS_RES_FULL_MARKER_NAME "ukey_resource_full.marker"
#define FATFS_RES_FULL_MARKER_PATH "0:/res/ukey_resource_full.marker"
#define FATFS_RES_PATH_MAX 320

typedef void (*fatfs_update_res_callback_t)(int current, int total,
                                            const char *status,
                                            const char *name);
...
int fatfs_update_res(bool dry_run, fatfs_update_res_callback_t callback,
                     int *moved_out, int *total_out) {
  if (moved_out != NULL) {
    *moved_out = 0;
  }
  if (total_out != NULL) {
  ...
  FILINFO marker = {0};
  FRESULT res = f_stat(FATFS_RES_FULL_MARKER_PATH, &marker);
  if (res == FR_NO_FILE || res == FR_NO_PATH) {
    return FATFS_UPDATE_RES_SKIPPED;
  }
  if (res != FR_OK) {
    return -(int)res;
  }

  res = f_mkdir(FATFS_RES_LIGHT_PATH);
  if (res != FR_OK && res != FR_EXIST) {
    return -(int)res;
  }
  res = f_mkdir(FATFS_RES_DARK_PATH);
  if (res != FR_OK && res != FR_EXIST) {
    return -(int)res;
  }
  res = f_mkdir(FATFS_RES_DEFAULT_PATH);
  if (res != FR_OK && res != FR_EXIST) {
    return -(int)res;
  }
  ...
  DIR dir = {0};
  FILINFO fno = {0};
  res = f_opendir(&dir, FATFS_RES_ROOT_PATH);
  if (res != FR_OK) {
    return -(int)res;
  }
  for (;;) {
    ...
    if ((fno.fattrib & AM_DIR) ||
        strcmp(fno.fname, FATFS_RES_RESOURCE_MARKER_NAME) == 0 ||
        strcmp(fno.fname, FATFS_RES_FULL_MARKER_NAME) == 0) {
      continue;
    }
    root_count++;
  }
  ...
  if (total_out != NULL) {
    *total_out = root_count;
  }
  if (dry_run) {
    return FATFS_UPDATE_RES_READY;
  }
  if (root_count == 0) {
    f_unlink(FATFS_RES_FULL_MARKER_PATH);
    return FATFS_UPDATE_RES_DONE;
  }

  if (callback != NULL) {
    callback(0, root_count, "Scanning resource files", NULL);
  }
  ...
  for (;;) {
    ...
    if (strcmp(fno.fname, FATFS_RES_RESOURCE_MARKER_NAME) == 0 ||
        strcmp(fno.fname, FATFS_RES_FULL_MARKER_NAME) == 0) {
      continue;
    }
    ...
    if (strncmp(fno.fname, "li_", 3) == 0) {
      target_dir = FATFS_RES_LIGHT_PATH;
      status = "Organizing light theme";
    } else if (strncmp(fno.fname, "da_", 3) == 0) {
      target_dir = FATFS_RES_DARK_PATH;
      status = "Organizing dark theme";
    } else {
      target_dir = FATFS_RES_DEFAULT_PATH;
      status = "Organizing default resources";
    }

    char old_path[FATFS_RES_PATH_MAX] = {0};
    char new_path[FATFS_RES_PATH_MAX] = {0};
    int old_len = snprintf(old_path, sizeof(old_path),
                           FATFS_RES_ROOT_PATH "/%s", fno.fname);
    int new_len =
        snprintf(new_path, sizeof(new_path), "%s/%s", target_dir, fno.fname);
    ...
    if (callback != NULL) {
      callback(moved, root_count, status, fno.fname);
    }
  ...
  if (moved < root_count) {
    return FATFS_UPDATE_RES_PENDING;
  }

  f_unlink(FATFS_RES_FULL_MARKER_PATH);
  if (callback != NULL) {
    callback(root_count, root_count, "Resource setup complete", NULL);
  }
  return FATFS_UPDATE_RES_DONE;
}

static mp_obj_t fatfs_update_res_callback_obj = mp_const_none;

STATIC void fatfs_update_res_mp_callback(int current, int total,
                                         const char *status, const char *name) {
  if (fatfs_update_res_callback_obj == mp_const_none) {
    return;
  }
  mp_obj_t args[4] = {
      mp_obj_new_int(current),
      mp_obj_new_int(total),
      mp_obj_new_str(status, strlen(status)),
      name == NULL ? mp_const_none : mp_obj_new_str(name, strlen(name)),
  };
  mp_call_function_n_kw(fatfs_update_res_callback_obj, 4, 0, args);
}

/// def update_res(callback=None, dry_run: bool=False) -> tuple[int, int, int]:
///     """
///     Organize flat resource packages into theme subdirectories.
///     """
STATIC mp_obj_t mod_trezorio_fatfs_update_res(size_t n_args,
                                              const mp_obj_t *args,
                                              mp_map_t *kwargs) {
  FATFS_ONLY_MOUNTED;
  enum { ARG_callback, ARG_dry_run };
  ...
  mp_obj_t callback_obj = vals[ARG_callback].u_obj;
  if (callback_obj != mp_const_none && !mp_obj_is_callable(callback_obj)) {
    mp_raise_TypeError("callback must be callable");
  }
  fatfs_update_res_callback_obj = callback_obj;
  fatfs_update_res_callback_t callback =
      fatfs_update_res_callback_obj == mp_const_none
          ? NULL
          : fatfs_update_res_mp_callback;
  int moved = 0;
  int total = 0;
  int status =
      fatfs_update_res(vals[ARG_dry_run].u_bool, callback, &moved, &total);
  fatfs_update_res_callback_obj = mp_const_none;
  if (status < 0) {
    FATFS_RAISE(FatFSError, -status);
  }

  mp_obj_t tuple[3] = {mp_obj_new_int(status), mp_obj_new_int(moved),
                       mp_obj_new_int(total)};
  return mp_obj_new_tuple(MP_ARRAY_SIZE(tuple), tuple);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_KW(mod_trezorio_fatfs_update_res_obj, 0,
                                  mod_trezorio_fatfs_update_res);
...
STATIC const mp_rom_map_elem_t mod_trezorio_fatfs_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_update_res),
     MP_ROM_PTR(&mod_trezorio_fatfs_update_res_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_UPDATE_RES_SKIPPED),
     MP_ROM_INT(FATFS_UPDATE_RES_SKIPPED)},
    {MP_ROM_QSTR(MP_QSTR_UPDATE_RES_PENDING),
     MP_ROM_INT(FATFS_UPDATE_RES_PENDING)},
    {MP_ROM_QSTR(MP_QSTR_UPDATE_RES_READY), MP_ROM_INT(FATFS_UPDATE_RES_READY)},
    {MP_ROM_QSTR(MP_QSTR_UPDATE_RES_DONE), MP_ROM_INT(FATFS_UPDATE_RES_DONE)},
};
