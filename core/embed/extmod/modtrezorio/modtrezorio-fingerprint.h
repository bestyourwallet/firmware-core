...
STATIC mp_obj_t mod_trezorio_fingerprint_register_template(mp_obj_t id) {
  ...
  if (id_ >= fpsensor_get_max_template_count()) {
...
STATIC mp_obj_t mod_trezorio_fingerprint_remove(mp_obj_t id) {
  uint16_t id_ = trezor_obj_get_uint(id);
...
STATIC mp_obj_t mod_trezorio_fingerprint_list_template(void) {
  ...
  uint16_t bitmap[5] = {0};
  uint8_t res =
      fingerprint_get_list(bitmap, sizeof(bitmap) / sizeof(bitmap[0]), &len);
  if (res != 0) {
    mp_raise_msg(&mp_type_RuntimeError, "Fingerprint sensor error");
  }
  uint8_t max_templates = fpsensor_get_max_template_count();
  mp_obj_tuple_t *tuple = MP_OBJ_TO_PTR(mp_obj_new_tuple(max_templates, NULL));
  for (int i = 0; i < max_templates; i++) {
    if (i < len) {
      tuple->items[i] = mp_obj_new_int_from_uint(bitmap[i]);
    } else {
