...
/// def ukey_firmware_hash() -> bytes:
///     """
///     Computes the sha256 hash of the firmware
///     """
STATIC mp_obj_t mod_trezorutils_ukey_firmware_hash(void) {
  vstr_t hash = {0};
...
STATIC MP_DEFINE_CONST_FUN_OBJ_0(mod_trezorutils_ukey_firmware_hash_obj,
                                 mod_trezorutils_ukey_firmware_hash);
...
/// def se_component_info(component_id: int) -> tuple[str, str, bytes, str, str,
/// bytes]:
///     """
///     Returns app and boot metadata for a logical component via the master SE.
///     """
STATIC mp_obj_t mod_trezorutils_se_component_info(mp_obj_t component_obj) {
  int component = mp_obj_get_int(component_obj);
  if (component < 0 || component >= SE_COMPONENT_COUNT) {
    mp_raise_ValueError("Invalid SE component id");
  }

  se_component_info_t info = {0};
#ifdef TREZOR_EMULATOR
  memcpy(info.version, "EMULATOR", 8);
  memcpy(info.build_id, "EMULATOR", 8);
  memcpy(info.boot_version, "EMULATOR", 8);
  memcpy(info.boot_build_id, "EMULATOR", 8);
#else
  if (sectrue != se_get_component_info((se_component_id_t)component, &info)) {
    mp_raise_msg(&mp_type_RuntimeError, "Failed to read SE component info");
  }
#endif

  mp_obj_t result[6] = {
      mp_obj_new_str_copy(&mp_type_str, (const uint8_t *)info.version,
                          strlen(info.version)),
      mp_obj_new_str_copy(&mp_type_str, (const uint8_t *)info.build_id,
                          strlen(info.build_id)),
      mp_obj_new_bytes(info.hash, sizeof(info.hash)),
      mp_obj_new_str_copy(&mp_type_str, (const uint8_t *)info.boot_version,
                          strlen(info.boot_version)),
      mp_obj_new_str_copy(&mp_type_str, (const uint8_t *)info.boot_build_id,
                          strlen(info.boot_build_id)),
      mp_obj_new_bytes(info.boot_hash, sizeof(info.boot_hash)),
  };

  return mp_obj_new_tuple(MP_ARRAY_SIZE(result), result);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mod_trezorutils_se_component_info_obj,
                                 mod_trezorutils_se_component_info);
...
MP_DEFINE_STR_OBJ(mp_UKEY_VERSION, UKEY_VERSION);
...
STATIC const mp_rom_map_elem_t mp_module_trezorutils_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_ukey_firmware_hash),
     MP_ROM_PTR(&mod_trezorutils_ukey_firmware_hash_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_se_component_info),
     MP_ROM_PTR(&mod_trezorutils_se_component_info_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_UKEY_VERSION), MP_ROM_PTR(&mp_UKEY_VERSION)},
...
#endif
#if USE_ACL16
    {MP_ROM_QSTR(MP_QSTR_USE_ACL16), mp_const_true},
#else
    {MP_ROM_QSTR(MP_QSTR_USE_ACL16), mp_const_false},
#endif
#if USE_ACL16_WALLET_CRYPTO
    {MP_ROM_QSTR(MP_QSTR_USE_ACL16_WALLET_CRYPTO), mp_const_true},
#else
    {MP_ROM_QSTR(MP_QSTR_USE_ACL16_WALLET_CRYPTO), mp_const_false},
#endif
