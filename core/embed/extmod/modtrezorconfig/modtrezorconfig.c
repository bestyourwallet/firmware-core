...
#define STORAGE_PRIVATE_FLAG (1UL << 31)
#define STORAGE_REGION_ALIGNMENT 4

static uint16_t storage_region_size(bool is_private) {
  return is_private ? PRIVATE_REGION_SIZE : PUBLIC_REGION_SIZE;
}

static secbool storage_region_access_ok(uint32_t key, uint32_t len,
                                        bool is_private) {
  uint16_t region_size = storage_region_size(is_private);
  if (key > UINT16_MAX || len > UINT16_MAX || key + len > region_size) {
    return secfalse;
  }
  if ((key % STORAGE_REGION_ALIGNMENT) != 0 ||
      (len % STORAGE_REGION_ALIGNMENT) != 0) {
    return secfalse;
  }
  return sectrue;
}
...
#error "Emulator not support SE_ACL16"
#endif
...
STATIC mp_obj_t mod_trezorconfig_get(size_t n_args, const mp_obj_t *args) {
  ...
  if (n_args < 4) {
    mp_raise_msg(&mp_type_ValueError, "Missing raw storage read length.");
  }
  uint32_t len = trezor_obj_get_uint(args[3]);
  bool is_private = key & STORAGE_PRIVATE_FLAG;
  secbool (*reader)(uint16_t, void *, uint16_t) =
      is_private ? se_get_private_region : se_get_public_region;

  key &= ~STORAGE_PRIVATE_FLAG;
  if (sectrue != storage_region_access_ok(key, len, is_private)) {
    mp_raise_msg(&mp_type_RuntimeError, "Invalid raw storage read.");
  }

  vstr_t vstr = {0};
  vstr_init_len(&vstr, len);
  vstr.len = len;
  if (sectrue != reader((uint16_t)key, vstr.buf, vstr.len)) {
    vstr_clear(&vstr);
    mp_raise_msg(&mp_type_RuntimeError, "Failed to get value from storage.");
  }
  return mp_obj_new_str_from_vstr(&mp_type_bytes, &vstr);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(mod_trezorconfig_get_obj, 2, 4,
...
STATIC mp_obj_t mod_trezorconfig_set(size_t n_args, const mp_obj_t *args) {
  ...
  key &= ~STORAGE_PRIVATE_FLAG;
  if (sectrue != storage_region_access_ok(key, value.len, is_private)) {
    mp_raise_msg(&mp_type_RuntimeError, "Could not save value");
  }

  if (sectrue != writer((uint16_t)key, value.buf, value.len)) {
    mp_raise_msg(&mp_type_RuntimeError, "Could not save value");
  }
...
STATIC mp_obj_t mod_trezorconfig_delete(size_t n_args, const mp_obj_t *args) {
  ...
  key &= ~STORAGE_PRIVATE_FLAG;
  if (sectrue !=
      storage_region_access_ok(key, STORAGE_REGION_ALIGNMENT, is_private)) {
    mp_raise_msg(&mp_type_RuntimeError, "Could not delete key");
  }

  uint8_t temp[STORAGE_REGION_ALIGNMENT] = {0};
  if (sectrue != writer((uint16_t)key, temp, sizeof(temp))) {
    mp_raise_msg(&mp_type_RuntimeError, "Could not delete key");
  }
  return mp_const_true;
}
...
///     """
///     Returns True if the device reset OTP flag is set.
///     """
STATIC mp_obj_t mod_trezorconfig_is_reset_flag_set(void) {
  return mp_obj_new_bool(sectrue ==
                         flash_otp_is_locked(FLASH_OTP_BLOCK_RESET_FLAG));
}

STATIC MP_DEFINE_CONST_FUN_OBJ_0(mod_trezorconfig_is_reset_flag_set_obj,
                                 mod_trezorconfig_is_reset_flag_set);
...
STATIC const mp_rom_map_elem_t mp_module_trezorconfig_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_is_reset_flag_set),
     MP_ROM_PTR(&mod_trezorconfig_is_reset_flag_set_obj)},
