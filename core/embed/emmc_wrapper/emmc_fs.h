...
{
    bool is_inited;
    bool is_mounted_ukey_data;
    FATFS fs_obj_ukeyData;
...
#define ExecuteCheck_OKEMMC_ADVANCED_MSG(func_call, display_msg, expected_condition, on_false) \
  {                                                                                            \
    strncpy(emmc_wrapper_status.ff_call, display_msg, 128);                                    \
    FRESULT EC_res = emmc_wrapper_status.ff_result = (func_call);                              \
    if ( !(expected_condition) )                                                               \
    {                                                                                          \
      on_false                                                                                 \
...
#define ExecuteCheck_OKEMMC_SIMPLE_MSG(func_call, display_msg)                                      \
  {                                                                                                 \
    ExecuteCheck_OKEMMC_ADVANCED_MSG(func_call, display_msg, (EC_res == FR_OK), { return false; }); \
  }
