...
#define UI_PROGRESS_PERCENTAGE_INVALID 0xffU
#define UI_PROGRESS_TIMEOUT_MS         (60U * 1000U)

static uint32_t ui_percentage_last = UI_PROGRESS_PERCENTAGE_INVALID;
static uint32_t ui_progress_bar_tick_last = 0;
static bool ui_progress_bar_active = false;

static void ui_progress_bar_state_reset(void)
{
    ui_percentage_last = UI_PROGRESS_PERCENTAGE_INVALID;
    ui_progress_bar_tick_last = 0;
    ui_progress_bar_active = false;
...
static int ui_progress_bar_handle_update(int percentage, const char* title)
{
    ...
    uint32_t percentage_current = (uint32_t)percentage;
    bool restarting_progress =
        (!ui_progress_bar_active) || (percentage_current == 0) ||
        ((ui_percentage_last != UI_PROGRESS_PERCENTAGE_INVALID) && (percentage_current < ui_percentage_last));

    if ( restarting_progress )
    {
        ui_progress_bar_visible_clear();
        ui_percentage_last = UI_PROGRESS_PERCENTAGE_INVALID;
    }

    if ( ui_progress_bar_active && ui_percentage_last == percentage_current )
    {
        return 0;
    }

    ui_percentage_last = percentage_current;
    ui_progress_bar_tick_last = HAL_GetTick();
    ui_progress_bar_active = true;
...
{
    bool was_active = ui_progress_bar_active;
...
void emmc_ui_progress_bar_timeout_poll(void)
{
    if ( !ui_progress_bar_active )
    {
        return;
    }

    if ( (HAL_GetTick() - ui_progress_bar_tick_last) < UI_PROGRESS_TIMEOUT_MS )
    {
        return;
    }
...
#define MCU_HEADER_MAGIC "UKTV"
#define SE_HEADER_MAGIC  "SE16"
...
static int check_file_contents(uint8_t iface_num, const uint8_t* buffer, uint32_t buffer_len)
{
    ...
    while ( buffer_len )
    {
                 ...
                 ) == sectrue )
            {
                     ...
                     ) == sectrue )
                {
                    ...
                    if ( memcmp(hash1, hash2, 32) == 0 )
                    {
                        ...
                        // compare version
                        if ( (version_compare(current_hdr.ukey_version, file_hdr.ukey_version) > 0) )
                        {
        ...
        else if ( memcmp(p_data, SE_HEADER_MAGIC, 4) == 0 )
        {
            ...
            ExecuteCheck_MSGS_ADV(
                load_acl16_image_header(
                    p_data, FIRMWARE_IMAGE_MAGIC_ACL16, FIRMWARE_IMAGE_MAXSIZE_ACL16, &acl16_hdr
                ),
                sectrue,
                {
            ...
            if ( (acl16_hdr.codelen + IMAGE_HEADER_SIZE) > buffer_len )
            {
            ...
            uint8_t index = 0;
            switch ( acl16_hdr.i2c_address << 1 )
            {
            case SE_1ST_ADDRESS:
            ...
            index = index * 2 + (acl16_hdr.firmware_type == SE_FIRMWARE_TYPE_APP_B ? 1 : 0);
            if ( index >= sizeof(update_info.se_location) )
            {
                send_failure(iface_num, FailureType_Failure_ProcessError, "SE index error!");
                return -1;
            }
            if ( update_info.se_location[index] )
            {
                send_failure(iface_num, FailureType_Failure_ProcessError, "SE firmware already exists!");
                return -1;
            }
            ...
            snprintf(
                update_info.items[update_info.item_count].new_version,
                sizeof(update_info.items[update_info.item_count].new_version), "%s",
                format_ver("%d.%d.%d", acl16_hdr.version)
            );

            if ( update_info.se_fw_type[index] == 0 )
            {
                update_info.items[update_info.item_count].type = UPDATE_SE;
                update_info.items[update_info.item_count].offset = p_data - buffer;
                update_info.items[update_info.item_count].length = acl16_hdr.codelen + IMAGE_HEADER_SIZE;
                update_info.item_count++;
                update_info.se_location[index] = update_info.item_count;
                update_info.se_address[index] = acl16_hdr.i2c_address << 1;
                update_info.se_fw_type[index] = acl16_hdr.firmware_type;
                update_info.se_count++;
            }
            p_data += acl16_hdr.codelen + IMAGE_HEADER_SIZE;
            buffer_len -= acl16_hdr.codelen + IMAGE_HEADER_SIZE;
            continue;
        }
        // BLE
...
int check_bootloader_update(image_header* file_hdr)
{
    ...
    if ( version_compare(VERSION_UINT32, file_hdr->version) > 0 )
        return 1;
...
static int
update_firmware_from_file(uint8_t iface_num, const char* path, bool check_only, bool require_confirm)
{
    ...
    if ( iface_num != USB_IFACE_NULL )
    {
        if ( require_confirm && !firmware_install_confirm() )
        {
    ...
    {
        uint8_t se_weights = (100 / update_info.item_count) * update_info.se_count;

        image_header_se se_hdr_a = {0};
        image_header_se se_hdr_b = {0};
        uint8_t* se_firmware_a_addr = NULL;
        uint8_t* se_firmware_b_addr = NULL;

        for ( int i = 0; i < (int)(sizeof(update_info.se_location) / sizeof(update_info.se_location[0]));
              i++ )
        {
            if ( update_info.se_location[i] == 0 )
            {
                continue;
            }
            ...
            image_header_se se_hdr;
            ExecuteCheck_MSGS_ADV(
                load_acl16_image_header(
                    p_data, FIRMWARE_IMAGE_MAGIC_ACL16, FIRMWARE_IMAGE_MAXSIZE_ACL16, &se_hdr
                ),
                sectrue,
                {
                    send_failure(iface_num, FailureType_Failure_ProcessError, "Update file header invalid!");
            ...
            if ( se_hdr.firmware_type == SE_FIRMWARE_TYPE_APP_A )
            {
                se_firmware_a_addr = p_data;
                memcpy(&se_hdr_a, &se_hdr, sizeof(image_header_se));
            }
            else if ( se_hdr.firmware_type == SE_FIRMWARE_TYPE_APP_B )
            {
                se_firmware_b_addr = p_data;
                memcpy(&se_hdr_b, &se_hdr, sizeof(image_header_se));
            }
        }
        ...
        uint8_t* se_firmware_data = NULL;
        image_header_se* p_se_hdr = NULL;

        uint8_t progress = 0;
        if ( !se_get_update_progress(&progress) )
        {
            send_failure(iface_num, FailureType_Failure_ProcessError, "SE get progress error");
            return -1;
        }
        if ( progress == SE_UPDATE_PARTITION_A )
        {
            se_firmware_data = se_firmware_a_addr;
            p_se_hdr = &se_hdr_a;
        }
        else if ( progress == SE_UPDATE_PARTITION_B )
        {
            se_firmware_data = se_firmware_b_addr;
            p_se_hdr = &se_hdr_b;
        }
        else
        {
            send_failure(iface_num, FailureType_Failure_ProcessError, "SE update progress error");
            return -1;
        }

        if ( se_firmware_data == NULL )
        {
            send_failure(iface_num, FailureType_Failure_ProcessError, "SE target slot firmware missing");
            return -1;
        }

        if ( p_se_hdr->i2c_address != 0 )
        {
            se_boot_set_address(p_se_hdr->i2c_address);
        }

        char target_ver[16] = {0};
        strncpy(target_ver, format_ver("%d.%d.%d", p_se_hdr->version), sizeof(target_ver) - 1);

        if ( !se_verify_firmware(se_firmware_data, IMAGE_HEADER_SIZE) )
        {
        ...
        snprintf(update_str, sizeof(update_str), "Installing SE %s", target_ver);
        ui_screen_install_title_clear();
        ui_screen_progress_bar_init(update_str, NULL, current_percent);

        // install
        if ( !se_update_firmware(
                 se_firmware_data + IMAGE_HEADER_SIZE, p_se_hdr->codelen, current_percent, se_weights,
                 ui_screen_install_progress_upload
             ) )
        {
        ...
        char* activated_ver = se_get_version_ex();
        if ( activated_ver == NULL )
        {
            send_failure(iface_num, FailureType_Failure_ProcessError, "SE get activated version error");
            return -1;
        }
        if ( strcmp(activated_ver, target_ver) != 0 )
        {
            send_failure(iface_num, FailureType_Failure_ProcessError, "SE version verify failed");
            return -1;
        }
    ...
    if ( update_info.mcu_location )
    {
        ...
        // ui confirm (skip in factory mode)
...
int check_firmware_from_file(uint8_t iface_num)
{
    ...
    if ( emmc_fs_file_read(UPDATE_INFO_FILE, 0, path_buffer, FF_MAX_LFN, &br) )
    {
        ...
        return update_firmware_from_file(iface_num, path_buffer, false, true);
    }
    return -1;
}

int process_msg_FirmwareUpdateEmmc(uint8_t iface_num, uint32_t msg_size, uint8_t* buf, bool factory_mode)
{
    MSG_INIT(msg_recv, FirmwareUpdateEmmc);
    MSG_RECV_RET_ON_ERR(msg_recv, FirmwareUpdateEmmc);

    bool require_confirm = !factory_mode;
    ...
    if ( ret == 0 )
    {
        ...
        ret = update_firmware_from_file(iface_num, msg_recv.path, true, require_confirm);
        if ( ret != 0 )
        {
            return ret;
        }
    ...
    ret = update_firmware_from_file(iface_num, msg_recv.path, false, require_confirm);
    if ( ret != 0 )
    {
        return ret;
    }
