...
#define COLOR_BL_DANGER RGB16(0xFF, 0x11, 0x00)  // ukey red
...
#define LOGO_SIZE 132
#define LOGO_OFFSET_X ((DISPLAY_RESX - LOGO_SIZE) / 2)
#define LOGO_OFFSET_Y (STATUS_BAR_HEIGHT + 92)

#define ICON_SIZE 128
#define ICON_OFFSET_X ((DISPLAY_RESX - ICON_SIZE) / 2)
#define ICON_OFFSET_Y (STATUS_BAR_HEIGHT + 92)
...
void ui_logo_ukey(void) {
  display_image(LOGO_OFFSET_X, LOGO_OFFSET_Y, LOGO_SIZE, LOGO_SIZE,
                toi_icon_ukey + 12, sizeof(toi_icon_ukey) - 12);
}

void ui_logo_warning(void) {
  display_image(ICON_OFFSET_X, ICON_OFFSET_Y, ICON_SIZE, ICON_SIZE,
...
void ui_icon_battery_dying(void) {
  int icon_width = 260;
  int icon_height = 190;
  int icon_offset_x = (DISPLAY_RESX - icon_width) / 2;
  int icon_offset_y = (DISPLAY_RESY - icon_height) / 2;
  display_image(icon_offset_x, icon_offset_y, icon_width, icon_height,
                toi_icon_battery_dying + 12,
                sizeof(toi_icon_battery_dying) - 12);
}
...
void ui_screen_boot(const vendor_header* const vhdr,
  ...
  const uint32_t fw_version = hdr->ukey_version;
...
void ui_screen_firmware_info(const vendor_header* const vhdr,
  ...
  if (vhdr && hdr) {
    ver_str = format_ver("Firmware %d.%d.%d by", (hdr->ukey_version));
  ...
  display_text_center(120, 220, "Go to ukey.com", -1, FONT_NORMAL, COLOR_BL_FG,
                      COLOR_BL_BG);
}
...
void ui_screen_fail_countdown(int restart_seconds) {
  char count_str[32];
  ui_screen_fail();
  mini_snprintf(count_str, sizeof(count_str), "Restarting in %d s",
                restart_seconds);
  display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY - 42, count_str, -1,
                      FONT_NORMAL, COLOR_BL_SUBTITLE, COLOR_BL_BG);
}
...
static int _ui_input_match(int zones, uint32_t evt) {
  ...
  // Factory Reset menu item. It moves up when Generate TRNG is hidden.
  if ((zones & INPUT_MENU_FACTORY_RESET) && x >= BOARD_OFFSET_X &&
      x < BOARD_OFFSET_X + BUTTON_FULL_WIDTH) {
    if ((zones & INPUT_MENU_GENERATE_TRNG) && y >= 369 && y < 429) {
      return INPUT_MENU_FACTORY_RESET;
    }
    if (!(zones & INPUT_MENU_GENERATE_TRNG) && y >= 276 && y < 336) {
      return INPUT_MENU_FACTORY_RESET;
    }
  }
  // Auto menu item
  if ((zones & INPUT_MENU_AUTO) && x >= BOARD_OFFSET_X &&
      x < BOARD_OFFSET_X + BUTTON_FULL_WIDTH && y >= 462 && y < 522) {
    return INPUT_MENU_AUTO;
  }
  return 0;
}
...
void ui_wipe_confirm(const image_header* const hdr) {
  ui_statusbar_update();
  ui_logo_warning();
  // if (hdr && (hdr->ukey_version != 0)) {
  //   const char *ver_str = format_ver("v%d.%d.%d", (hdr->ukey_version));
...
void ui_install_confirm(image_header* current_hdr,
  ...
  display_text_center(DISPLAY_RESX / 2, TITLE_OFFSET_Y, "System Update", -1,
                      FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, SUBTITLE_OFFSET_Y,
                      "Install firmware by UKey?", -1, FONT_NORMAL,
                      COLOR_BL_SUBTITLE, COLOR_BL_BG);
  ...
  const char* ver_str = format_ver("%d.%d.%d", current_hdr->ukey_version);
  display_text_right(DISPLAY_RESX / 2 - 25, SUBTITLE_OFFSET_Y + 90, ver_str, -1,
                     FONT_NORMAL, COLOR_WHITE, COLOR_BL_PANEL);
  ver_str = format_ver("%d.%d.%d", new_hdr->ukey_version);
...
void ui_install_acl16_confirm(const char* old_ver, const char* boot_ver) {
  char str[128] = {0};
  ui_statusbar_update();
  ui_logo_ukey();
...
static void draw_update_section(update_info_t update_info, int offset_y,
  ...
  //    if (update_info.se_location[i] == 0) continue;
  //
  //    display_bar_radius_ex(BUTTON_LEFT_OFFSET_X, offset_y, BUTTON_FULL_WIDTH,
  //                          bar_height, COLOR_BL_PANEL, COLOR_BL_BG,
  //                          bar_height / 2);
...
void ui_bootloader_first(const image_header* const hdr) {
  ...
  display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY - 92, "UKeyOS", -1,
                      FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);
  if (current_hdr) {
    const char* ver_str = format_ver("%d.%d.%d", (current_hdr->ukey_version));
  ...
  if (se_state != 0) {
    const int se_warning_y = SUBTITLE_OFFSET_Y + 80;

    strcat(se_info, "SE ");
    strcat(se_info, "in boot");
    display_text_center(DISPLAY_RESX / 2, se_warning_y, se_info, -1,
                        FONT_NORMAL, COLOR_BL_SUBTITLE, COLOR_BL_BG);
    display_text_center(DISPLAY_RESX / 2, se_warning_y + 30,
                        "please install se firmware", -1, FONT_NORMAL,
                        COLOR_BL_SUBTITLE, COLOR_BL_BG);
  }
}
...
void ui_bootloader_main_menu(const image_header* const hdr) {
  ...
  ui_statusbar_update();
  display_bar_radius_ex(BOARD_OFFSET_X, 60, BUTTON_FULL_WIDTH, 276,
                        COLOR_BL_PANEL, COLOR_BL_BG, BUTTON_RADIUS);
  ...
  // Generate TRNG is hidden for now. Keep it at the end so it can be
  // restored by uncommenting this block and adding INPUT_MENU_GENERATE_TRNG
  // back to the page 5 input zones.
  // display_bar(0, offset_y, DISPLAY_RESX, 3, COLOR_BLACK);
  // offset_y += offset_line;
  // offset_y += off_n;
...
BOOT_TARGET ui_bootloader_start_menu(BOOT_TARGET boot_target) {
  ui_bootloader_page_current = 5;

  int offset_x = 32, offset_y = 90, off_n = 28, offset_line = 30;
  ui_statusbar_update();
  display_bar_radius_ex(BOARD_OFFSET_X, 60, BUTTON_FULL_WIDTH, 452,
                        COLOR_BL_PANEL, COLOR_BL_BG, BUTTON_RADIUS);
  offset_y += offset_line;
  display_text(offset_x, offset_y, "BOARD MODE", -1, FONT_PJKS_BOLD_26,
               COLOR_BL_FG, COLOR_BL_PANEL);
  offset_y += offset_line;
  ...
  offset_y += offset_line;
  offset_y += off_n;
  display_text(offset_x, offset_y, "UPDATE FIRMWARE", -1, FONT_PJKS_BOLD_26,
               COLOR_BL_FG, COLOR_BL_PANEL);
  ...
  offset_y += offset_line;
  offset_y += off_n;
  display_text(offset_x, offset_y, "FIRMWARE MODE", -1, FONT_PJKS_BOLD_26,
               COLOR_BL_FG, COLOR_BL_PANEL);
  offset_y += offset_line;
  display_bar(0, offset_y, DISPLAY_RESX, 3, COLOR_BLACK);
  offset_y += offset_line;
  offset_y += off_n;
  display_text(offset_x, offset_y, "USB MSC", -1, FONT_PJKS_BOLD_26,
               COLOR_BL_FG, COLOR_BL_PANEL);
  offset_y += offset_line;
  display_bar(0, offset_y, DISPLAY_RESX, 3, COLOR_BLACK);
  offset_y += offset_line;
  ...
  char auto_mode_str[64] = {0};
  strcat(auto_mode_str, "AUTO MODE (");
  if (boot_target == BOOT_TARGET_NORMAL) {
    strcat(auto_mode_str, "NORMAL");
  } else if (boot_target == BOOT_TARGET_MSC) {
    strcat(auto_mode_str, "MSC");
  } else if (boot_target == BOOT_TARGET_BOARDLOADER) {
    strcat(auto_mode_str, "BOARDLOADER");
  } else if (boot_target == BOOT_TARGET_BOOTLOADER) {
    strcat(auto_mode_str, "BOOTLOADER");
  }
  strcat(auto_mode_str, ")");
  display_text(offset_x, offset_y, auto_mode_str, -1, FONT_PJKS_BOLD_26,
               COLOR_BL_FG, COLOR_BL_PANEL);
  offset_y += offset_line;
  ...
  display_bar_radius_ex(BUTTON_LEFT_OFFSET_X, BUTTON_OFFSET_Y,
                        BUTTON_FULL_WIDTH, BUTTON_HEIGHT, COLOR_BL_FG,
                        COLOR_BL_BG, BUTTON_RADIUS);
  display_text_center(DISPLAY_RESX / 2, 755, "REBOOT", -1, FONT_PJKS_BOLD_26,
                      COLOR_BL_BG, COLOR_BL_FG);

  while (1) {
    int response = ui_input_poll(
        INPUT_PREVIOUS | INPUT_MENU_DEVICE_INFO | INPUT_MENU_GENERATE_TRNG |
            INPUT_MENU_REBOOT | INPUT_MENU_FACTORY_RESET | INPUT_MENU_AUTO,
        false);
    if (INPUT_PREVIOUS == response) {
      ...
      *BOOT_TARGET_FLAG_ADDR = BOOT_TARGET_BOARDLOADER;
      ...
      emmc_fs_path_info("0:data/fw_p2.bin", &path_info);
      emmc_fs_file_read("0:data/fw_p2.bin", 0,
                        (uint32_t*)FMC_SDRAM_FIRMWARE_P2_ADDRESS,
                        path_info.size, &processed_len);
      *DEBUG_PORT_FLAG_ADDR = DEBUG_PORT_RTT;
      display_clear();
      bus_fault_disable();
      mpu_config_firmware(sectrue, sectrue);
      jump_to(FIRMWARE_START + 1024 + 2560);
    } else if (INPUT_MENU_REBOOT == response) {
      display_clear();
      return BOOT_TARGET_BOOTLOADER;
    } else if (INPUT_MENU_FACTORY_RESET == response) {
      display_clear();
      return BOOT_TARGET_MSC;
    } else if (INPUT_MENU_AUTO == response) {
      display_clear();
      *DEBUG_PORT_FLAG_ADDR = DEBUG_PORT_VCP;
...
void ui_bootloader_view_details(const image_header* const hdr) {
  ...
  offset_y += offset_line;
  display_text(offset_x, offset_y, "UKey Core 26", -1, FONT_NORMAL, COLOR_BL_FG,
               COLOR_BL_PANEL);
  offset_y += offset_seg;
  ...
  offset_y += offset_line;
  if (hdr && hdr->ukey_version != 0) {
    ver_str = format_ver("%d.%d.%d", (hdr->ukey_version));
...
void ui_bootloader_factory_reset_confirm(void) {
  ...
  const int title_y = ICON_OFFSET_Y + ICON_SIZE + 56;
  const int subtitle_y = title_y + 38 + 38 + 20;

  display_text(12, title_y, "Are you sure you want to", -1, FONT_PJKS_BOLD_38,
               COLOR_BL_FG, COLOR_BL_BG);
  display_text(12, title_y + 38, "factory reset the device?", -1,
               FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);

  display_text(12, subtitle_y, "Please keep your Secret Recovery", -1,
               FONT_PJKS_BOLD_26, COLOR_BL_SUBTITLE, COLOR_BL_BG);
  display_text(12, subtitle_y + 30, "Phrase handy to recover access", -1,
               FONT_PJKS_BOLD_26, COLOR_BL_SUBTITLE, COLOR_BL_BG);
  display_text(12, subtitle_y + 60, "to your wallet.", -1, FONT_PJKS_BOLD_26,
               COLOR_BL_SUBTITLE, COLOR_BL_BG);
  ...
  display_bar_radius_ex(BUTTON_RIGHT_OFFSET_X, BUTTON_OFFSET_Y,
                        ...
                        COLOR_BL_BG, BUTTON_RADIUS);
  display_text_center(DISPLAY_RESX - DISPLAY_RESX / 4, 755, "Continue", -1,
                      FONT_PJKS_BOLD_26, COLOR_BL_BG, COLOR_BL_DANGER);
}

void ui_bootloader_factory_reset_final_confirm(void) {
  ui_bootloader_page_current = 7;
  ...
  display_text_center(DISPLAY_RESX / 2, title_y, "Final confirmation", -1,
                      FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, title_y + 38, "Backup required", -1,
                      FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);

  display_text_center(DISPLAY_RESX / 2, subtitle_y,
                      "Make sure your Secret Recovery", -1, FONT_PJKS_BOLD_26,
                      COLOR_BL_SUBTITLE, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, subtitle_y + 30,
                      "Phrase is safely backed up.", -1, FONT_PJKS_BOLD_26,
                      COLOR_BL_SUBTITLE, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, subtitle_y + 60,
                      "Without it, wallet access can", -1, FONT_PJKS_BOLD_26,
                      COLOR_BL_SUBTITLE, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, subtitle_y + 90, "be permanently lost.",
                      -1, FONT_PJKS_BOLD_26, COLOR_BL_SUBTITLE, COLOR_BL_BG);

  display_bar_radius_ex(BUTTON_LEFT_OFFSET_X, BUTTON_OFFSET_Y,
                        BUTTON_HALF_WIDTH, BUTTON_HEIGHT, COLOR_BL_DANGER,
                        COLOR_BL_BG, BUTTON_RADIUS);
  display_text_center(DISPLAY_RESX / 4, 755, "Reset", -1, FONT_PJKS_BOLD_26,
                      COLOR_BL_BG, COLOR_BL_DANGER);

  display_bar_radius_ex(BUTTON_RIGHT_OFFSET_X, BUTTON_OFFSET_Y,
                        BUTTON_HALF_WIDTH, BUTTON_HEIGHT, COLOR_BL_DARK,
                        COLOR_BL_BG, BUTTON_RADIUS);
  display_text_center(DISPLAY_RESX - DISPLAY_RESX / 4, 755, "Cancel", -1,
...
void ui_bootloader_page_switch(const image_header* const hdr) {
  ...
  if (ui_bootloader_page_current == 0) {
    ...
    response = ui_input_poll(INPUT_PREVIOUS | INPUT_MENU_DEVICE_INFO |
                                 INPUT_MENU_REBOOT | INPUT_MENU_FACTORY_RESET,
                             false);
    if (INPUT_PREVIOUS == response) {
      display_clear();
      ui_bootloader_first(hdr);
    ...
    if (INPUT_CANCEL == response) {
      ...
      display_clear();
      ui_bootloader_factory_reset_final_confirm();
    }
  } else if (ui_bootloader_page_current == 7) {
    response = ui_input_poll(INPUT_CANCEL | INPUT_RESTART, false);
    if (INPUT_RESTART == response) {
      display_clear();
      ui_bootloader_main_menu(hdr);
    } else if (INPUT_CANCEL == response) {
      // The final reset button is on the left to avoid fast double-tap erase.
      ui_fadeout();
      ui_screen_wipe();
      ui_fadein();
