...
#define FIRMWARE_UPDATE_EMMC_FAIL_RESTART_SECONDS 10
#define FIRMWARE_UPDATE_EMMC_FAIL_POLL_MS 10
#define FIRMWARE_UPDATE_EMMC_FAIL_POLLS_PER_SECOND \
  (1000 / FIRMWARE_UPDATE_EMMC_FAIL_POLL_MS)
  ...
  for (;;) {
    while (true) {
      ...
      // no packet, no pwer button pressed
        ...
        emmc_ui_progress_bar_timeout_poll();
    ...
    switch (msg_id) {
      ...
      case MSG_NAME_TO_ID(Ping):                // Ping
        ...
        break;
      case MSG_NAME_TO_ID(GetHardwareHash):     // GetHardwareHash
        process_msg_GetHardwareHash(USB_IFACE_NUM, msg_size, buf);
        break;
      ...
      case MSG_NAME_TO_ID(FirmwareUpdateEmmc):  // FirmwareUpdateEmmc
        r = process_msg_FirmwareUpdateEmmc(USB_IFACE_NUM, msg_size, buf, false);
        if (r < 0 && r != -4) {  // error
          ui_fadeout();
          ui_screen_fail_countdown(FIRMWARE_UPDATE_EMMC_FAIL_RESTART_SECONDS);
          ui_fadein();
          bool restart_requested = false;
          for (int seconds = FIRMWARE_UPDATE_EMMC_FAIL_RESTART_SECONDS; seconds > 0 && !restart_requested; seconds--) {
            ui_screen_fail_countdown(seconds);
            for (int i = 0; i < FIRMWARE_UPDATE_EMMC_FAIL_POLLS_PER_SECOND; i++) {
              if (touch_click()) {
                restart_requested = true;
                break;
              }
              hal_delay(FIRMWARE_UPDATE_EMMC_FAIL_POLL_MS);
            }
          }
          bluetooth_reset();
      ...
      case MSG_NAME_TO_ID(EmmcDirRemove):       // EmmcDirRemove
        ...
        break;
      case MSG_NAME_TO_ID(UkeyGetFeatures):   // UkeyGetFeatures
        process_msg_UkeyGetFeatures(USB_IFACE_NUM, msg_size, buf, vhdr, hdr);
        break;
      default:
...
static bool factory_command_allowed(uint16_t msg_id) {
  switch (msg_id) {
    case MSG_NAME_TO_ID(Initialize):
    case MSG_NAME_TO_ID(Ping):
    case MSG_NAME_TO_ID(GetFeatures):
    case MSG_NAME_TO_ID(GetHardwareHash):
    case MSG_NAME_TO_ID(GetDeviceInfo):
    case MSG_NAME_TO_ID(FactoryGetChallenge):
    case MSG_NAME_TO_ID(FactoryAuthenticate):
    case MSG_NAME_TO_ID(Reboot):
      return true;
    case MSG_NAME_TO_ID(DeviceInfoSettings):
    case MSG_NAME_TO_ID(WriteSEPrivateKey):
    case MSG_NAME_TO_ID(ReadSEPublicKey):
    case MSG_NAME_TO_ID(WriteSEPublicCert):
    case MSG_NAME_TO_ID(ReadSEPublicCert):
      return factory_auth_is_authenticated();
    default:
      return false;
  }
}
...
secbool bootloader_usb_loop_factory(const vendor_header* const vhdr,
  ...
  for (;;) {
    ...
    if (!factory_command_allowed(msg_id)) {
      process_msg_factory_forbidden(USB_IFACE_NUM, msg_size, buf);
      continue;
    }
    factory_auth_note_activity();
    ...
    switch (msg_id) {
      ...
      case MSG_NAME_TO_ID(GetHardwareHash):  // GetHardwareHash
        process_msg_GetHardwareHash(USB_IFACE_NUM, msg_size, buf);
        break;
      case MSG_NAME_TO_ID(FactoryGetChallenge):  // FactoryGetChallenge
        process_msg_FactoryGetChallenge(USB_IFACE_NUM, msg_size, buf);
        break;
      case MSG_NAME_TO_ID(FactoryAuthenticate):  // FactoryAuthenticate
        process_msg_FactoryAuthenticate(USB_IFACE_NUM, msg_size, buf);
        break;
      ...
      case MSG_NAME_TO_ID(Reboot):  // Reboot
        ...
        break;
      case MSG_NAME_TO_ID(FirmwareUpdateEmmc):  // FirmwareUpdateEmmc
        process_msg_FirmwareUpdateEmmc(USB_IFACE_NUM, msg_size, buf, true);
        break;
      case MSG_NAME_TO_ID(EmmcFixPermission):  // EmmcFixPermission
        process_msg_EmmcFixPermission(USB_IFACE_NUM, msg_size, buf);
        break;
...
void battery_level_check() {
  bool boot_ok = ble_battery_ok();
  if (!boot_ok) {
    ui_icon_battery_dying();
    HAL_Delay(1000);
    ble_power_off();
    display_clear();
  }
...
void boot_file_init(void) {
  if (!emmc_fs_path_exist("0:/boot")) {
    emmc_fs_dir_make("0:/boot");
  }
}
...
int main(void) {
  ...
  // display_clear();
  display_bar_radius(127, 683, 226, 6, COLOR_BLACK, COLOR_BLACK, 4);
  ...
  if (se_mode == 0 && (!device_serial_set() || !se_has_cerrificate())) {
    display_clear();
  ...
  // boot_target = ui_bootloader_start_menu(boot_target);
  UNUSED(ui_bootloader_start_menu);
