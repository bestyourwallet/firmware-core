...
static HAL_StatusTypeDef I2C_IsAcknowledgeFailed(I2C_HandleTypeDef *hi2c, uint32_t Timeout, uint32_t Tickstart) {
  ...
  if (__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_AF) == SET) {
    ...
    /* Flush TX register to match HAL recovery behavior after AF/NACK. */
...
HAL_StatusTypeDef i2c_master_send(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t Timeout) {
  uint32_t tickstart;
  uint8_t data[2];
  uint8_t xor = 0;
...
int i2c_master_recive(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t *Size, uint32_t Timeout) {
    ...
    while (1) {
      ...
      if (data_len > *Size) {
        *Size = 0;
        ...
        ret = I2C_RECV_BUFFER_TOO_SMALL;
        break;
      }

      if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_TCR, RESET, Timeout, 0) != HAL_OK) {
        DEBUG_BREAK();
        return HAL_ERROR;
      }
      ...
      while (temp_len > 0) {
        ...
        if ((temp_len > 0) && (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_TCR, RESET, Timeout, 0) != HAL_OK)) {
          DEBUG_BREAK();
          return HAL_ERROR;
        }
      }

      if ((data_len > 0) && (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_TCR, RESET, Timeout, 0) != HAL_OK)) {
        DEBUG_BREAK();
        return HAL_ERROR;
      }
...
static secbool _se_transmit_ex(uint8_t addr, uint8_t *cmd, uint16_t len, uint8_t *resp, uint16_t *resp_len) {
  delay_ms(2);
  int ret = 0;
  char err_info[64] = {0};
...
secbool se_transmit_ex(uint8_t addr, uint8_t *cmd, uint16_t len, uint8_t *resp, uint16_t *resp_len) {
  uint32_t irq = disable_irq();
  se_irq_nest++;
  secbool result = _se_transmit_ex(addr, cmd, len, resp, resp_len);
  se_irq_nest--;
  if (se_irq_nest == 0) {
    enable_irq(irq);
  }
...
secbool se_transmit(uint8_t *cmd, uint16_t len, uint8_t *resp, uint16_t *resp_len) {
  return se_transmit_ex(SE_MASTER_ADDRESS, cmd, len, resp, resp_len);
}
...
int se_master_send(uint8_t *cmd, uint16_t len) {
  uint32_t irq = disable_irq();
  HAL_StatusTypeDef result = i2c_master_send(&i2c_handle_se, SE_1ST_ADDRESS, cmd, len, 500);
  enable_irq(irq);
  return result;
}
...
int se_master_recv(uint8_t *resp, uint16_t *resp_len) {
  uint32_t irq = disable_irq();
  int ret = i2c_master_recive(&i2c_handle_se, SE_1ST_ADDRESS, resp, resp_len, I2C_RECV_TIMEOUT);
  enable_irq(irq);
  return ret;
}
