...
static void dcmi_init()
{
    ...
    */
    ...
    NVIC_SetPriority(DMA2_Stream1_IRQn, IRQ_PRI_DCMI_DMA);
...
bool camera_start(uint8_t* buffer_address, uint32_t mode)
{
    HAL_DCMI_Stop(&DCMI_Handle);
    camera_opened = false;
    capture_done = false;
    if ( HAL_DCMI_Start_DMA(&DCMI_Handle, mode, (uint32_t)buffer_address, (WIN_W * WIN_H) / 2) != HAL_OK )
    {
        return false;
    }
    camera_opened = true;
...
bool camera_capture_start(void)
{
    ...
    return camera_start((uint8_t*)CAM_BUF_ADDRESS, DCMI_MODE_SNAPSHOT);
#else
    return camera_start((uint8_t*)CAM_BUF_ADDRESS, DCMI_MODE_CONTINUOUS);
#endif
}
