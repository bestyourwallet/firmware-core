...
#define FACTORY_RDP_LEVEL (OB_RDP_LEVEL_1)
#else
#define WANT_RDP_LEVEL (OB_RDP_LEVEL_0)
#define WANT_WRP_SECTORS (0)
#define FACTORY_RDP_LEVEL (OB_RDP_LEVEL_0)
#endif
 ...
 *            HCLK(Hz)                       = 200000000 (CM4 CPU, AXI and AHBs Clock) AHB Prescaler = 2 
...
void flash_option_bytes_factory_init(void) {
  FLASH_OBProgramInitTypeDef ob_config;
  ...
  if (ob_config.RDPLevel == OB_RDP_LEVEL_1 &&
      FACTORY_RDP_LEVEL == OB_RDP_LEVEL_0) {
    return;
  }

  bool need_update = false;

  if (ob_config.RDPLevel != FACTORY_RDP_LEVEL) {
    ob_config.OptionType = OPTIONBYTE_RDP | OPTIONBYTE_USER | OPTIONBYTE_BOR;

    ob_config.RDPLevel = FACTORY_RDP_LEVEL;
    ob_config.BORLevel = OB_BOR_LEVEL0;
    ...
    ob_config.OptionType = 0;

    if (ob_config.BORLevel != OB_BOR_LEVEL0) {
      ob_config.OptionType |= OPTIONBYTE_BOR;
      ob_config.BORLevel = OB_BOR_LEVEL0;
      need_update = true;
    }
