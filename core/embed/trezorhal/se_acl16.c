...
#define PIN_PASSPHRASE_MAX_ITEMS 15
...
// clang-format off
static void xor_cal(uint8_t *data1, uint8_t *data2, uint16_t len, uint8_t * xor) {
  uint16_t i;
...
static secbool se_get_rand_ex(uint8_t addr, uint8_t *rand, uint16_t rand_len) {
  ...
  return se_transmit_ex(addr, rand_cmd, sizeof(rand_cmd), rand, &resp_len);
}

secbool se_get_rand(uint8_t *rand, uint16_t rand_len) {
  return se_get_rand_ex(SE_MASTER_ADDRESS, rand, rand_len);
}
...
static secbool se_reset_se_ex(uint8_t addr) {
  ...
  secbool result = se_transmit_ex(addr, cmd, sizeof(cmd), NULL, &resp_len);
...
secbool se_reset_se(void) { return se_reset_se_ex(SE_MASTER_ADDRESS); }
...
static secbool se_transmit_mac_ex(uint8_t addr, uint8_t *session_key,
  ...
  if (!se_transmit_ex(addr, APDU, data_len, se_recv_buffer, &se_recv_len)) {
...
secbool se_transmit_mac(uint8_t ins, uint8_t p1, uint8_t p2, uint8_t *data,
  ...
  se_irq_nest++;
  secbool result = se_transmit_mac_ex(SE_MASTER_ADDRESS, se_session_key, ins,
...
secbool se_random_encrypted_ex(uint8_t addr, uint8_t *session_key,
  ...
  for (int retry = 0; retry < 3; retry++) {
    ...
    ret = se_transmit_ex(addr, cmd, sizeof(cmd), se_recv_buffer, &recv_len);
    if (ret == sectrue) {
      break;
    }
  }

  ensure(ret, "se_transmit_ex failed");
...
secbool se_sync_session_key_ex_old(uint8_t addr, uint8_t *session_key) {
  ...
  ensure(flash_otp_read(FLASH_OTP_BLOCK_ACL16_SESSION_KEY, 0, default_key, 16),
         NULL);
  ...
  if (!se_transmit_ex(addr, sync_cmd, sizeof(sync_cmd), data_buf,
                         &recv_len)) {
...
static void get_pubkey(uint8_t addr, uint8_t *pubkey) {
  ...
  switch (addr) {
    case SE_MASTER_ADDRESS:
      otp_pubkey_1 = FLASH_OTP_BLOCK_SE_1_PUBKEY1;
      otp_pubkey_2 = FLASH_OTP_BLOCK_SE_1_PUBKEY2;
      break;
    default:
      return;
  }
...
static secbool se_sync_session_key_ex(uint8_t addr, uint8_t *session_key) {
  ...
  if (!se_transmit_ex(addr, sync_cmd, sizeof(sync_cmd), signature,
                         &recv_len)) {
    memset(session_key, 0x00, SESSION_KEYLEN);
    return secfalse;
  }
...
static secbool _se_sync_session_key(void) {
  if (sectrue == se_sync_session_key_ex(SE_MASTER_ADDRESS, se_session_key)) {
...
static secbool _se_reset_storage(void) {
  ...
  uint8_t state = 0;
  if (!se01_get_state(&state)) {
    return secfalse;
  }

  if (state != SE_STATE_APP) {
    return secfalse;
  }
...
static secbool se_set_reset_flag(void) {
  if (sectrue == flash_otp_is_locked(FLASH_OTP_BLOCK_RESET_FLAG)) {
    return sectrue;
  }

  uint8_t buf[FLASH_OTP_BLOCK_SIZE] = {0};
  const char reset_flag[] = "device reset";
  memcpy(buf, reset_flag, sizeof(reset_flag) - 1);

  if (sectrue !=
      flash_otp_write(FLASH_OTP_BLOCK_RESET_FLAG, 0, buf,
                      FLASH_OTP_BLOCK_SIZE)) {
    return secfalse;
  }

  return flash_otp_lock(FLASH_OTP_BLOCK_RESET_FLAG);
}

secbool se_reset_storage(void) {
  if (sectrue != _se_reset_storage()) {
    return secfalse;
  }
  // se_fp_reset_storage();
  return se_set_reset_flag();
}
...
static int _se_get_ver_info(uint8_t addr, uint8_t cmd, uint8_t *out,
  ...
  if (!se_transmit_ex(addr, get_ver, sizeof(get_ver), out, &len)) {
    memset(out, 0, in_len);
    return 0;
  }
  return len;
...
/*
 * P1 is a logical component identifier. The command is always sent through
 * se_transmit(), which fixes the physical destination to SE_MASTER_ADDRESS.
 * P2 retains the field identifiers used by the existing 0xF7 command.
 */
static int _se_get_component_ver_info(se_component_id_t component,
                                      uint8_t field, uint8_t *out,
                                      uint16_t out_len) {
  uint8_t get_ver[5] = {0x00, 0xF7, (uint8_t)component, field, 0x00};
  uint16_t len = out_len;

  if (component >= SE_COMPONENT_COUNT || out == NULL || out_len == 0) {
    return 0;
  }

  if (!se_transmit(get_ver, sizeof(get_ver), out, &len)) {
...
secbool se_get_component_info(se_component_id_t component,
                              se_component_info_t *info) {
  if (component >= SE_COMPONENT_COUNT || info == NULL) {
    return secfalse;
  }

  memzero(info, sizeof(*info));

  int version_len = _se_get_component_ver_info(
      component, 0x00, (uint8_t *)info->version, SE_COMPONENT_VERSION_SIZE);
  int build_id_len = _se_get_component_ver_info(
      component, 0x01, (uint8_t *)info->build_id, SE_COMPONENT_BUILD_ID_SIZE);
  int hash_len = _se_get_component_ver_info(
      component, 0x02, info->hash, sizeof(info->hash));
  int boot_version_len = _se_get_component_ver_info(
      component, 0x03, (uint8_t *)info->boot_version,
      SE_COMPONENT_VERSION_SIZE);
  int boot_build_id_len = _se_get_component_ver_info(
      component, 0x04, (uint8_t *)info->boot_build_id,
      SE_COMPONENT_BUILD_ID_SIZE);
  int boot_hash_len = _se_get_component_ver_info(
      component, 0x05, info->boot_hash, sizeof(info->boot_hash));

  if (version_len <= 0 || version_len > SE_COMPONENT_VERSION_SIZE ||
      build_id_len <= 0 || build_id_len > SE_COMPONENT_BUILD_ID_SIZE ||
      hash_len != SE_COMPONENT_HASH_SIZE || boot_version_len <= 0 ||
      boot_version_len > SE_COMPONENT_VERSION_SIZE || boot_build_id_len <= 0 ||
      boot_build_id_len > SE_COMPONENT_BUILD_ID_SIZE ||
      boot_hash_len != SE_COMPONENT_HASH_SIZE) {
    memzero(info, sizeof(*info));
    return secfalse;
  }
...
char *se01_get_version(void) {
  ...
  int len = se_get_version(SE_1ST_ADDRESS, version, sizeof(version));
  if (len == 0) {
    return NULL;
  }
  return version;
...
char *se01_get_build_id(void) {
  ...
  int len = se_get_build_id(SE_1ST_ADDRESS, build_id, sizeof(build_id));
  if (len == 0) {
    return NULL;
  }
  return build_id;
...
uint8_t *se01_get_hash(void) {
  ...
  if (se_get_hash(SE_1ST_ADDRESS, hash, sizeof(hash)) == 0) {
    return NULL;
  }
  hash_init = true;
  return hash;
...
char *se01_get_boot_version(void) {
  ...
  int len = se_get_boot_version(SE_1ST_ADDRESS, version, sizeof(version));
  if (len == 0) {
    return NULL;
  }
  return version;
...
char *se01_get_boot_build_id(void) {
  ...
  int len = se_get_boot_build_id(SE_1ST_ADDRESS, build_id, sizeof(build_id));
  if (len == 0) {
    return NULL;
  }
  return build_id;
...
uint8_t *se01_get_boot_hash(void) {
  ...
  if (se_get_boot_hash(SE_1ST_ADDRESS, hash, sizeof(hash)) == 0) {
    return NULL;
  }
  hash_init = true;
  return hash;
...
secbool se_get_ecdh_pubkey(uint8_t addr, uint8_t *key) {
  ...
  return se_transmit_ex(addr, cmd, sizeof(cmd), key, &resp_len);
}

secbool se_lock_ecdh_pubkey(uint8_t addr) {
  uint8_t cmd[6] = {0x00, 0xF5, 0x00, 0x05, 0x01, 0x02};
  return se_transmit_ex(addr, cmd, sizeof(cmd), NULL, NULL);
}

secbool se_is_ecdh_pubkey_locked(uint8_t addr) {
  uint8_t cmd[6] = {0x00, 0xF5, 0x00, 0x05, 0x01, 0x03};
  uint8_t resp[1] = {0};
  uint16_t resp_len = 1; 
  se_transmit_ex(addr, cmd, sizeof(cmd), resp, &resp_len);
  return resp[0] == 0x55 ? sectrue : secfalse;
}
...
secbool se_write_certificate(const uint8_t *cert, uint16_t len) {
  uint8_t cmd[1024 + 7] = {0x00, 0xF6, 0x00, 0x01, 0x00};
  uint16_t cmd_len = 0;
  uint16_t resp_len = 0;
  if (cert == NULL || len == 0 || len > 1024) {
    return secfalse;
  }
  if (len > 255) {
    cmd[4] = 0x00;
...
secbool se_set_private_key_extern(uint8_t key[32]) {
  ...
  return se_transmit(set_key, sizeof(set_key), NULL, &resp_len);
}
...
secbool se_set_session_key_ex(uint8_t addr, const uint8_t *session_key) {
  ...
  return se_transmit_ex(addr, cmd, 21, NULL, &resp_len);
}

secbool se_set_session_key(const uint8_t *session_key) {
  memcpy(se_session_key, session_key, SESSION_KEYLEN);
  ensure(se_set_session_key_ex(SE_MASTER_ADDRESS, session_key), "se set session key failed");
  return sectrue;
}
...
secbool se_hasPin(void) {
  secbool result = se_hasPin_ex(SE_MASTER_ADDRESS, se_session_key);
  return result;
}
...
secbool se_setPin(const char *pin) {
  secbool result = se_setPin_ex(SE_MASTER_ADDRESS, se_session_key, pin);
  return result;
}
...
secbool se_verifyPin(const char *pin, pin_type_t pin_type) {
  secbool result = se_verifyPin_ex(SE_MASTER_ADDRESS, se_session_key, pin, pin_type);
  if (result == sectrue) {
    return sectrue;
  } else {
...
secbool se_changePin(const char *oldpin, const char *newpin) {
  ...
  secbool result = se_changePin_ex(SE_MASTER_ADDRESS, se_session_key, oldpin, newpin);
  return result;
}
...
secbool se_getRetryTimes(uint8_t *ptimes) {
  uint8_t retry_cnts = 0;
  ensure(se_getRetryTimes_ex(SE_MASTER_ADDRESS, se_session_key, &retry_cnts),
...
secbool se_set_pin_passphrase(const char *pin, const char *passphrase_pin,
  ...
  secbool result =
      se_set_pin_passphrase_ex(SE_MASTER_ADDRESS, se_session_key, pin,
...
secbool se_delete_pin_passphrase(const char *passphrase_pin, bool *current) {
  secbool result = se_delete_pin_passphrase_ex(
      SE_MASTER_ADDRESS, se_session_key, passphrase_pin, current);
  if (result == sectrue) {
    if (result == sectrue) {
      return sectrue;
    }
...
secbool se_change_pin_passphrase(const char *old_pin, const char *new_pin) {
  secbool result = se_change_pin_passphrase_ex(
      SE_MASTER_ADDRESS, se_session_key, old_pin, new_pin);
  return result;
}

secbool se_get_pin_passphrase_current(char *passphrase, uint8_t *passphrase_len) {
  uint16_t resp_len = PASSPHRASE_MAX_LENGTH + 2;
  uint8_t resp[2 + PASSPHRASE_MAX_LENGTH];  // [STATE:1] [LEN:1] [PASSPHRASE:LEN]
  if (!se_transmit_mac(SE_INS_PIN, 0x00, 0x0F, NULL, 0, resp, &resp_len)) {
    return secfalse;
  }
  ...
  *passphrase_len = resp[1];
  if (*passphrase_len > 0) {
    memcpy(passphrase, resp + 2, *passphrase_len);
  }
  return sectrue;
}
...
secbool se_clearSecsta(void) {
  ensure(se_clearSecsta_ex(SE_MASTER_ADDRESS, se_session_key),
...
secbool se_getSecsta(void) {
  secbool result = se_getSecsta_ex(SE_MASTER_ADDRESS, se_session_key);
  return result;
}
...
secbool session_generate_seed_percent(uint8_t *percent) {
  ...
  if (!se_transmit(cmd, sizeof(cmd), percent, &recv_len)) {
    sw1sw2 = se_last_error();
...
secbool se_fp_write(uint32_t offset, const void *val_dest, uint32_t len, uint8_t index, uint8_t total) {
  uint8_t cmd[8] = {0};
  ...
  while (len) {
    ...
    memcpy(APDU_DATA + 8, (uint8_t *)val_dest + packet_offset, packet_len);
    if (!se_transmit_mac(SE_INS_WRITE_DATA, 0x00, 0x02, APDU_DATA, 8 + packet_len, NULL, NULL)) {
      return secfalse;
    }
...
secbool se_fp_read(uint32_t offset, void *val_dest, uint32_t len, uint8_t index, uint8_t total) {
  uint8_t cmd[8] = {0};
  ...
  while (len) {
    ...
    cmd[6] = (packet_len >> 8) & 0xFF;
    cmd[7] = packet_len & 0xFF;
    if (!se_transmit_mac(SE_INS_READ_DATA, 0x00, 0x02, cmd, 8, (uint8_t *)val_dest + packet_offset, &packet_len)) {
      return secfalse;
    }
...
secbool se_u2f_gen_handle_and_node(const uint8_t app_id[32],
  ...
  cmd[4] = 32;
  if (!se_transmit(cmd, 5 + 32, (uint8_t *)recv, &recv_len)) {
    return secfalse;
  }
