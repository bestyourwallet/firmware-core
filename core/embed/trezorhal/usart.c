...
UART_HandleTypeDef huart_se;

void se_uart_init(void) {
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  ...
  if (HAL_RCCEx_PeriphCLKConfig(&PeriphClkInitStruct) != HAL_OK) {
    ensure(secfalse, "huart_se HAL_RCCEx_PeriphCLKConfig failed");
  }
  ...
  /* UART7 init */
  huart_se.Instance = UART7;
  huart_se.Init.BaudRate = BAUD_RATE;
  huart_se.Init.WordLength = UART_WORDLENGTH_8B;
  huart_se.Init.StopBits = UART_STOPBITS_1;
  huart_se.Init.Parity = UART_PARITY_NONE;
  huart_se.Init.HwFlowCtl = UART_HWCONTROL_NONE;
  huart_se.Init.Mode = UART_MODE_TX_RX;
  huart_se.Init.OverSampling = UART_OVERSAMPLING_16;
  huart_se.Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
  huart_se.Init.ClockPrescaler = UART_PRESCALER_DIV1;
  huart_se.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;

  if (HAL_UART_Init(&huart_se) != HAL_OK) {
    ensure(secfalse, "huart_se init failed");
  }
}