...
#define FIRMWARE_IMAGE_MAGIC_ACL16 0x36314553  // SE16
#define FIRMWARE_IMAGE_MAXSIZE_ACL16 (370 * 1024)
...
typedef struct {
  ...
  uint8_t __sigmask;
  uint8_t __sig[64];
} __attribute__((packed)) image_header_se;
...
typedef struct {
  ...
  uint8_t se_location[8];
  uint8_t se_address[8];
  uint8_t se_fw_type[8];
...
secbool __wur load_acl16_image_header(const uint8_t* const data,
                                      const uint32_t magic,
                                      const uint32_t maxsize,
