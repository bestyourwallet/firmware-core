...
_Static_assert(sizeof(CTAP_credential_id_storage) == FIDO2_RESIDENT_CREDENTIALS_SIZE, "CTAP_credential_id_storage size must be flash page size");
...
secbool se_transmit_mac(uint8_t ins, uint8_t p1, uint8_t p2, uint8_t *data, uint16_t data_len, uint8_t *recv, uint16_t *recv_len);
...
secbool se_random_encrypted_ex(uint8_t addr, uint8_t *session_key, uint8_t *rand, uint16_t len);
...
#define SE_COMPONENT_COUNT 4
#define SE_COMPONENT_VERSION_SIZE 16
#define SE_COMPONENT_BUILD_ID_SIZE 8
#define SE_COMPONENT_HASH_SIZE 32
...
typedef struct {
  char version[SE_COMPONENT_VERSION_SIZE + 1];
  char build_id[SE_COMPONENT_BUILD_ID_SIZE + 1];
  uint8_t hash[SE_COMPONENT_HASH_SIZE];
  char boot_version[SE_COMPONENT_VERSION_SIZE + 1];
  char boot_build_id[SE_COMPONENT_BUILD_ID_SIZE + 1];
  uint8_t boot_hash[SE_COMPONENT_HASH_SIZE];
} se_component_info_t;

/*
 * Reads the metadata for a logical SE component through the master SE.
 * The component identifier is not an I2C address.
 */
...
secbool se_import_slip39(const uint8_t *master_secret, uint8_t len, uint8_t backup_type, uint16_t identifier, uint8_t iteration_exponent);
...
secbool se_set_public_region(uint16_t offset, const void *val_dest, uint16_t len);
secbool se_get_public_region(uint16_t offset, void *val_dest, uint16_t len);
secbool se_set_private_region(uint16_t offset, const void *val_dest, uint16_t len);
...
secbool se_sign_message_with_write_key(uint8_t *msg, uint32_t msg_len, uint8_t *signature);
...
secbool se_derive_keys(HDNode *out, const char *curve, const uint32_t *address_n, size_t address_n_count, uint32_t *fingerprint);
secbool se_derive_xmr_key(const char *curve, const uint32_t *address_n, size_t address_n_count, uint8_t *pubkey, uint8_t *prikey_hash);
secbool se_derive_xmr_private_key(const uint8_t *pubkey, const uint32_t index, uint8_t *prikey);
secbool se_xmr_get_tx_key(const uint8_t *rand, const uint8_t *hash, uint8_t *out);
secbool se_node_sign_digest(const uint8_t *hash, uint8_t *sig, uint8_t *by);
int se_ecdsa_sign_digest(const uint8_t curve, const uint8_t canonical, const uint8_t *digest, uint8_t *sig, uint8_t *pby);

int se_secp256k1_sign_digest(const uint8_t canonical, const uint8_t *digest, uint8_t *sig, uint8_t *pby);
...
int se_aes256_encrypt(const uint8_t *data, uint16_t data_len, const uint8_t *iv, uint8_t *value, uint16_t value_len, uint8_t *out);
int se_aes256_decrypt(const uint8_t *data, uint16_t data_len, const uint8_t *iv, uint8_t *value, uint16_t value_len, uint8_t *out);

int se_nem_aes256_encrypt(const uint8_t *ed25519_public_key, const uint8_t *iv, const uint8_t *salt, uint8_t *payload, uint16_t size, uint8_t *out);
int se_nem_aes256_decrypt(const uint8_t *ed25519_public_key, const uint8_t *iv, const uint8_t *salt, uint8_t *payload, uint16_t size, uint8_t *out);
int se_slip21_node(uint8_t *data);
int se_slip21_fido_node(uint8_t *data);

secbool se_authorization_set(const uint32_t authorization_type, const uint8_t *authorization, uint32_t authorization_len);
secbool se_authorization_get_type(uint32_t *authorization_type);
secbool se_authorization_get_data(uint8_t *authorization_data, uint32_t *authorization_len);
...
secbool se_u2f_register(const uint8_t app_id[32], const uint8_t challenge[32], uint8_t key_handle[64], uint8_t pub_key[65], uint8_t sign[64]);
secbool se_u2f_gen_handle_and_node(const uint8_t app_id[32], uint8_t key_handle[64], HDNode *out);
secbool se_u2f_validate_handle(const uint8_t app_id[32], const uint8_t key_handle[64]);
secbool se_u2f_authenticate(const uint8_t app_id[32], const uint8_t key_handle[64], const uint8_t challenge[32], uint8_t *u2f_counter, uint8_t sign[64]);
secbool se_derive_fido_keys(HDNode *out, const char *curve, const uint32_t *address_n, size_t address_n_count, uint32_t *fingerprint);
...
secbool se_get_fido2_resident_credentials(uint32_t index, uint8_t *dest, uint16_t *dst_len);
secbool se_set_fido2_resident_credentials(uint32_t index, const uint8_t *src, uint16_t len);
