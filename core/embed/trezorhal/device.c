...
void device_verify_ble(void) {
  ...
  if (compare_str_version(ble_ver, "1.1.0") < 0) {
    strcat(info, "current ble version: ");
    strcat(info, ble_ver);
    display_text_center(
      DISPLAY_RESX / 2, DISPLAY_RESY / 2, "Please upgrade the Bluetooth.", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK
    );
    display_text_center(
      DISPLAY_RESX / 2, DISPLAY_RESY / 2 + 30, "Touch the screen to continue", -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK
    );
    display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY / 2 + 60, info, -1, FONT_NORMAL, COLOR_WHITE, COLOR_BLACK);
    while (!touch_click())
      ;
    return;
  }
  ...
  if (secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_BLE_PUBKEY1) ||
    ...
    ensure(flash_otp_write(FLASH_OTP_BLOCK_BLE_PUBKEY1, 0, pubkey + 1, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_write(FLASH_OTP_BLOCK_BLE_PUBKEY2, 0, pubkey + 33, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_lock(FLASH_OTP_BLOCK_BLE_PUBKEY1), NULL);
    ensure(flash_otp_lock(FLASH_OTP_BLOCK_BLE_PUBKEY2), NULL);
  } else {
    ensure(flash_otp_read(FLASH_OTP_BLOCK_BLE_PUBKEY1, 0, pubkey + 1, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_read(FLASH_OTP_BLOCK_BLE_PUBKEY2, 0, pubkey + 33, FLASH_OTP_BLOCK_SIZE), NULL);
  }
  pubkey[0] = 0x04;
  random_buffer(rand_buf, sizeof(rand_buf));
  ensure(ble_sign_msg(rand_buf, sizeof(rand_buf), signature) ? sectrue : secfalse, NULL);
  sha256_Raw(rand_buf, 16, digest);

  ensure(ecdsa_verify_digest(&secp256k1, pubkey, signature, digest) == 0 ? sectrue : secfalse, NULL);
}
...
void device_para_init(void) {
  ...
  uint8_t pubkey[64];
  if (secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_SE_1_PUBKEY1) ||
      secfalse == flash_otp_is_locked(FLASH_OTP_BLOCK_SE_1_PUBKEY2)) {
    ensure(se_get_ecdh_pubkey(SE_MASTER_ADDRESS, pubkey), NULL);
    ensure(se_lock_ecdh_pubkey(SE_MASTER_ADDRESS), NULL);
    ensure(flash_otp_write(FLASH_OTP_BLOCK_SE_1_PUBKEY1, 0, pubkey, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_write(FLASH_OTP_BLOCK_SE_1_PUBKEY2, 0, pubkey + 32, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_lock(FLASH_OTP_BLOCK_SE_1_PUBKEY1), NULL);
    ensure(flash_otp_lock(FLASH_OTP_BLOCK_SE_1_PUBKEY2), NULL);
  }

#if !BOOT_ONLY
  if (se_is_ecdh_pubkey_locked(SE_MASTER_ADDRESS) != sectrue) {
    ensure(se_lock_ecdh_pubkey(SE_MASTER_ADDRESS), NULL);
  }
#endif
  ...
  if (!flash_otp_is_locked(FLASH_OTP_RANDOM_KEY)) {
    ...
    ensure(flash_otp_write(FLASH_OTP_RANDOM_KEY, 0, entropy, FLASH_OTP_BLOCK_SIZE), NULL);
    ensure(flash_otp_lock(FLASH_OTP_RANDOM_KEY), NULL);
  }
  dev_info.random_key_init = true;
  ensure(flash_otp_read(FLASH_OTP_RANDOM_KEY, 0, dev_info.random_key, FLASH_OTP_BLOCK_SIZE), NULL);
  ...
  if (flash_otp_is_locked(FLASH_OTP_CPU_FIRMWARE_INFO)) {
    ...
    strlcpy(
      dev_info.pre_firmware,
      (char *)(flash_otp_data->flash_otp[FLASH_OTP_CPU_FIRMWARE_INFO] + FLASH_OTP_BLOCK_SIZE / 2),
  ...
  strlcpy(dev_info.serial, (char *)flash_otp_data->flash_otp[FLASH_OTP_DEVICE_SERIAL], sizeof(dev_info.serial));
...
bool device_set_serial(char *dev_serial) {
  ...
  // check serial
  if (!flash_otp_is_locked(FLASH_OTP_DEVICE_SERIAL)) {
    if (check_all_ones(flash_otp_data->flash_otp[FLASH_OTP_DEVICE_SERIAL], FLASH_OTP_BLOCK_SIZE)) {
      strlcpy((char *)buffer, dev_serial, sizeof(buffer));
      ensure(flash_otp_write(FLASH_OTP_DEVICE_SERIAL, 0, buffer, FLASH_OTP_BLOCK_SIZE), NULL);
...
bool device_cpu_firmware_set(void) {
  if ((0 < strlen(dev_info.cpu_info) && strlen(dev_info.cpu_info) < FLASH_OTP_BLOCK_SIZE / 2) &&
      (0 < strlen(dev_info.pre_firmware) && strlen(dev_info.pre_firmware) < FLASH_OTP_BLOCK_SIZE / 2)) {
    return true;
  }
  return false;
}
...
bool device_set_cpu_firmware(char *cpu_info, char *firmware_ver) {
  ...
  // check serial
  if (!flash_otp_is_locked(FLASH_OTP_CPU_FIRMWARE_INFO)) {
    if (check_all_ones(flash_otp_data->flash_otp[FLASH_OTP_CPU_FIRMWARE_INFO], FLASH_OTP_BLOCK_SIZE)) {
      strlcpy((char *)buffer, cpu_info, sizeof(buffer) / 2);
      strlcpy((char *)buffer + FLASH_OTP_BLOCK_SIZE / 2, firmware_ver, sizeof(buffer) / 2);
      ensure(flash_otp_write(FLASH_OTP_CPU_FIRMWARE_INFO, 0, buffer, FLASH_OTP_BLOCK_SIZE), NULL);
...
void device_get_stm32_unique_id(uint8_t *id) {
  uint32_t st_id[3];
  st_id[0] = HAL_GetUIDw0();
  st_id[1] = HAL_GetUIDw1();
  st_id[2] = HAL_GetUIDw2();
  memcpy(id, st_id, sizeof(st_id));
}
...
bool usb_msc_mode(void) {
  int ui_res = 0;
  ui_generic_confirm_simple("USB MSC Mode");
...
static bool _motor_test(void) {
  ...
  while (1) {
    ...
    motor_play_slide();
    dwt_delay_ms(1000);
    // motor_resonant_finder(dur_f, dur_r, 0);
    dwt_delay_ms(delay);
    ui_res = ui_response_ex();
    if (ui_res != UI_RESPONSE_NONE) {
      motor_reset();
...
static bool _mifare_test(void) {
  int ui_res = 0;
  ui_generic_confirm_simple("MIFARE test");

  uint8_t aes_key[16] = {0};
  password_to_aes_key((const char *)"000000", aes_key);

  nfc_init();
  nfc_pwr_ctl(true);
  while (1) {
    if (nfc_poll_card()) {
      int res = aes_authenticate(aes_key, 0);
      if (!res) {
        printf("aes_authenticate faild\n");
      }
    }
    hal_delay(500);
...
static bool _flashled_test(void) {
  uint32_t start, current, led_state;
  int ui_res = 0;
...
static bool _fp_test(void) {
  ...
  while (1) {
    ...
    uint16_t width = 0;
    uint16_t height = 0;
    if (fingerprint_capture_image(&image_data, &width, &height) == 0) {
      display_fp(196, 500, width, height, (uint8_t *)image_data);
    }
    ui_res = ui_response_ex();
    if (ui_res != UI_RESPONSE_NONE) {
      return ui_res == UI_RESPONSE_YES;
...
bool se_test(void) __attribute__((weak));

FUN_NO_OPTMIZE
static bool _se_test(void) {
  if (se_test == NULL) {
    printf("[SE_TEST][FAIL] se_test.c is not linked\n");
    return false;
  }
  return se_test();
}
...
void device_test(bool force) {
  ...
  // --- SE ACL Test ---
  if (_se_test()) {
    display_text(0, y_pos, "SE ACL test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  } else {
    display_text(0, y_pos, "SE ACL test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  //   display_text(0, y_pos, "SE test faild", -1, FONT_NORMAL, COLOR_RED,
  //   COLOR_BLACK); while (1);
  // } else {
  //   display_text(0, y_pos, "SE test done", -1, FONT_NORMAL, COLOR_WHITE,
  //   COLOR_BLACK);
  // }
  // y_pos += LINE_STEP;
  ...
  //     display_text(0, y_pos, "SPI-FLASH test faild", -1, FONT_NORMAL,
  //     COLOR_RED, COLOR_BLACK); while (1);
  //   } else {
  //     display_text(0, y_pos, "SPI-FLASH test done", -1, FONT_NORMAL,
  //     COLOR_WHITE, COLOR_BLACK);
  //   }
  //   y_pos += LINE_STEP;
  // }
  ...
  if (emmc_get_capacity_in_bytes() == 0) {
    display_text(0, y_pos, "EMMC test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  } else {
    display_text(0, y_pos, "EMMC test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  }
  y_pos += LINE_STEP;
  ...
  // --- MOTOR Test ---
  if (_motor_test()) {
    display_text(0, y_pos, "MOTOR test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  } else {
    display_text(0, y_pos, "MOTOR test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  // --- CAMERA Test ---
  if (_camera_test()) {
    display_text(0, y_pos, "CAMERA test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  } else {
    display_text(0, y_pos, "CAMERA test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  // --- Mifare Test ---
  if (_mifare_test()) {
    display_text(0, y_pos, "Mifare test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  } else {
    display_text(0, y_pos, "Mifare test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  // --- FINGERPRINT Test ---
  if (_fp_test()) {
    display_text(0, y_pos, "FINGERPRINT test done", -1, FONT_NORMAL,
                 COLOR_WHITE, COLOR_BLACK);
  } else {
    display_text(0, y_pos, "FINGERPRINT test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  // --- FLASHLED Test ---
  if (_flashled_test()) {
    display_text(0, y_pos, "FLASHLED test done", -1, FONT_NORMAL, COLOR_WHITE,
                 COLOR_BLACK);
  } else {
    display_text(0, y_pos, "FLASHLED test faild", -1, FONT_NORMAL, COLOR_RED,
                 COLOR_BLACK);
    while (1)
      ;
  }
  ...
  // ensure(flash_otp_write(FLASH_OTP_FACTORY_TEST, 0, buf,
  // FLASH_OTP_BLOCK_SIZE), NULL);
...
void device_burnin_test(bool force) {
  ...
  do {
    ...
    if (index != index_bak) {
      ...
      switch (index) {
        case 0:
          ...
          if (jped_decode("res/wallpaper_1.jpg",
                          FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0) {
            display_print_clear();
            display_printf("show wallpaper_1.jpg err\n");
          }
          break;
        case 1:
          if (jped_decode("res/wallpaper_2.jpg",
                          FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0) {
            display_print_clear();
            display_printf("show wallpaper_2.jpg err\n");
          }
          break;
        case 2:
          if (jped_decode("res/wallpaper_3.jpg",
                          FMC_SDRAM_LVGL_BUFFER_ADDRESS) != 0) {
            display_print_clear();
            display_printf("show wallpaper_3.jpg err\n");
          }
          break;
        case 3:
          display_clear();
          ...
          if (fingerprint_detect) {
            display_fp(10, 620, image_width, image_height, image_data);
    ...
    if ((current - previous) > (TIMER_1S / 2)) {
      ...
      fingerprint_detect = false;
      const uint8_t *captured_image = NULL;
      if (fingerprint_capture_image(&captured_image, &image_width,
                                    &image_height) == 0 &&
          captured_image != NULL &&
          (size_t)image_width * image_height <= sizeof(image_data)) {
        memcpy(image_data, captured_image, (size_t)image_width * image_height);
...
bool device_overwrite_se_ecdh_public(uint8_t *pubkey) {
  ...
  if (memcmp(flash_otp_data->flash_otp[FLASH_OTP_BLOCK_SE_1_PUBKEY1], pubkey,
             FLASH_OTP_BLOCK_SIZE) == 0 &&
      memcmp(flash_otp_data->flash_otp[FLASH_OTP_BLOCK_SE_1_PUBKEY2],
             pubkey + FLASH_OTP_BLOCK_SIZE, FLASH_OTP_BLOCK_SIZE) == 0) {
    return true;
  }
  ...
  memcpy(otp_data_buffer_p->flash_otp[FLASH_OTP_BLOCK_SE_1_PUBKEY1], pubkey,
         FLASH_OTP_BLOCK_SIZE);
  memcpy(otp_data_buffer_p->flash_otp[FLASH_OTP_BLOCK_SE_1_PUBKEY2],
         pubkey + FLASH_OTP_BLOCK_SIZE, FLASH_OTP_BLOCK_SIZE);
...
bool device_overwrite_ble_public(uint8_t *pubkey) {
  ...
  if (memcmp(flash_otp_data->flash_otp[FLASH_OTP_BLOCK_BLE_PUBKEY1], pubkey,
             FLASH_OTP_BLOCK_SIZE) == 0 &&
      memcmp(flash_otp_data->flash_otp[FLASH_OTP_BLOCK_BLE_PUBKEY2],
             pubkey + FLASH_OTP_BLOCK_SIZE, FLASH_OTP_BLOCK_SIZE) == 0) {
    return true;
  }
  ...
  memcpy(otp_data_buffer_p->flash_otp[FLASH_OTP_BLOCK_BLE_PUBKEY1], pubkey,
         FLASH_OTP_BLOCK_SIZE);
  memcpy(otp_data_buffer_p->flash_otp[FLASH_OTP_BLOCK_BLE_PUBKEY2],
         pubkey + FLASH_OTP_BLOCK_SIZE, FLASH_OTP_BLOCK_SIZE);
