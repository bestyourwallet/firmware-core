...
uint8_t dev_pwr_raw_sta = BLE_REMOVE_POWER;
...
bool ble_battery_ok(void) {
  for (uint8_t attempt = 0; attempt < 5; attempt++) {
    uint16_t voltage = 0;
    uint8_t counter = 0;
    bool voltage_ok = ble_get_battery_voltage(&voltage);

    get_ble_charging = false;
    ble_get_dev_info();

    while (!get_ble_charging) {
      ble_uart_poll();
      counter++;
      hal_delay(1);
      if (counter > 100) {
    ...
    if (voltage_ok && voltage != 0 && get_ble_charging) {
      if (voltage < 3500 && dev_pwr_raw_sta == BLE_REMOVE_POWER) {
        return false;
      }
      return true;
    }
...
void ble_pair_warning(void) {
  ...
  display_text(8, 241, "firmware.ukey.com  on  your  computer", -1, FONT_NORMAL,
               COLOR_WHITE, COLOR_BLACK);
...
void ble_uart_poll(void) {
  ...
  switch (ble_usart_msg.cmd) {
    ...
    case BLE_CMD_POWER_STA:
      ...
      dev_pwr_raw_sta = ble_usart_msg.cmd_vale[0];
