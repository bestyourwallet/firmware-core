...
secbool load_image_header(const uint8_t* const data, const uint32_t magic,
  ...
  memcpy(&hdr->ukey_version, data + 24, 4);
...
secbool load_acl16_image_header(const uint8_t* const data, const uint32_t magic,
                                const uint32_t maxsize,
                                image_header_se* const hdr) {
  uint16_t offset = 0;
  memcpy(&hdr->magic, data + offset, 4);
  offset += 4;
  if (hdr->magic != magic) return secfalse;

  memcpy(&hdr->hdrlen, data + offset, 4);
  offset += 4;
  // if (hdr->hdrlen != IMAGE_HEADER_SIZE) return secfalse;

  memcpy(&hdr->expiry, data + offset, 4);
  offset += 4;

  memcpy(&hdr->version, data + offset, 4);
  offset += 4;

  memcpy(&hdr->i2c_address, data + offset, 1);
  offset += 1;

  memcpy(&hdr->firmware_type, data + offset, 1);
  offset += 1;

  memcpy(&hdr->codelen, data + offset, 4);
  offset += 4;

  memcpy(hdr->hashes, data + offset, 512);
  offset += 512;

  memcpy(hdr->sig1, data + offset, 64);
  offset += 64;
...
secbool read_vendor_header(const uint8_t* const data,
  ...
  if (vhdr->magic != 0x56544B55) return secfalse;  // UKTV
...
void vendor_header_hash(const vendor_header* const vhdr, uint8_t* hash) {
  ...
  blake2s_Update(&ctx, "UKey Vendor Header", 18);
  blake2s_Final(&ctx, hash, BLAKE2S_DIGEST_LENGTH);
}
