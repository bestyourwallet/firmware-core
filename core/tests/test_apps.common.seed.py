...
from apps.common.seed import Slip21Node, _get_seed_passphrase
...
class TestSeed(unittest.TestCase):
    ...
    def test_get_seed_passphrase_uses_user_input_without_passphrase_pin(self):
        with patch("apps.common.seed.get_passphrase", return_value="typed-passphrase"):
            with patch("apps.common.passphrase.is_passphrase_pin_enabled", return_value=False):
                self.assertEqual(
                    await_result(_get_seed_passphrase(wire.DUMMY_CONTEXT)),
                    "typed-passphrase",
                )

    def test_get_seed_passphrase_uses_se_value_with_passphrase_pin(self):
        with patch("apps.common.passphrase.is_passphrase_pin_enabled", return_value=True):
            with patch(
                "trezorcrypto.se_acl16.get_pin_passphrase_current",
                return_value="bound-passphrase",
            ):
                self.assertEqual(
                    await_result(_get_seed_passphrase(wire.DUMMY_CONTEXT)),
                    "bound-passphrase",
    ...
    def test_get_seed_passphrase_falls_back_to_empty_on_se_failure(self):
        with patch("apps.common.passphrase.is_passphrase_pin_enabled", return_value=True):
            with patch(
                "trezorcrypto.se_acl16.get_pin_passphrase_current",
                return_value=None,
