...
class TestStorageCache(unittest.TestCase):
    ...
    def test_resume_session_updates_current_session_id(self):
        session_id1 = cache.start_session()
        session_id2 = cache.start_session()

        cache.start_session(session_id1)
        self.assertEqual(cache.get_session_id(), session_id1)

        cache.start_session(session_id2)
        self.assertEqual(cache.get_session_id(), session_id2)

    def test_end_session_clears_current_session_id(self):
        cache.start_session()
        self.assertTrue(cache.get_session_id())

        cache.end_current_session()

        self.assertEqual(cache.get_session_id(), b"")
    ...
    @mock_storage
    def test_Initialize_software_mode_does_not_call_acl16_passphrase_state_check(self):
        self.assertFalse(utils.USE_ACL16_WALLET_CRYPTO)

        original_check = base.se_acl16.check_passphrase_btc_test_address

        def fail_if_called(_address):
            raise AssertionError(
                "software mode should not call ACL16 passphrase state validation"
            )

        base.se_acl16.check_passphrase_btc_test_address = fail_if_called
        try:
            session_id = cache.start_session()
            features = await_result(
                handle_Initialize(
                    DUMMY_CONTEXT,
                    Initialize(
                        session_id=session_id,
                        passphrase_state="muBhqTZU8EVSE2JHz43WGmdZJXKp1s2Sz4",
                    ),
                )
            )
            self.assertEqual(features.session_id, session_id)
        finally:
            base.se_acl16.check_passphrase_btc_test_address = original_check
