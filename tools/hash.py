...
def compute_boardloader_hashes(data):
    data = data[:-BOARD_INFO_SIZE] + b"\x00" * BOARD_INFO_SIZE
...
def main(args):
    ...
    elif args.bintype == "boardloader":
        data = open(args.path, "rb").read()
        hash = compute_boardloader_hashes(data)
        print("boardloader version hash: ", hash.hex())
    else:
        print("no support")
