...
class PyrightTool:
    ...
    def check_input_correctness(self) -> None:
        ...
        # Resolve relative paths in file_specific_ignores to absolute paths
        resolved: FileSpecificIgnores = {}
        for file, ignores in self.file_specific_ignores.items():
            abs_path = str((self.workdir / file).resolve())
            resolved[abs_path] = ignores
        self.file_specific_ignores = resolved
    ...
    def should_ignore_file_specific_error(self, file: str, error: Error) -> bool:
        ...
        for ignore_object in self.file_specific_ignores[file]:
            if ignore_object.rule:
                if error.get("rule", "") == ignore_object.rule:
