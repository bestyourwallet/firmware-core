...
ASSETS_DEFAULT_DIR = (
    ROOT.parent / "core" / "src" / "trezor" / "lvglui" / "assets" / "default"
)
ASSETS_CHAIN_DIR = ASSETS_DEFAULT_DIR / "chain"
...
def _chain_asset_icon(
    group: str, name: str, default_name: str, allow_generic: bool = False
) -> str:
    filename = f"{name}.png"
    if (ASSETS_CHAIN_DIR / group / filename).exists():
        return f"chain_{group}_{filename}"
    if allow_generic and (ASSETS_CHAIN_DIR / filename).exists():
        return f"chain_{filename}"
    return f"chain_{group}_{default_name}.png"
...
def _load_btc_coins() -> Coins:
    ...
    for file in DEFS_DIR.glob("bitcoin/*.json"):
        ...
        coin.update(
            ...
            icon=_chain_asset_icon("btc", coin["coin_shortcut"].lower(), "btc"),
            primary_color=int(
                coin.get("primary_color", "0x255770" if not is_testnet else "0xF54927"),
                16,
            ),
        )
        coins.append(coin)
...
def _load_ethereum_networks() -> Coins:
    ...
    for chain in sorted(
        ...
        network = dict(
            ...
            key=f"eth:{shortcut}",
            icon=_chain_asset_icon(
                "evm",
                EVM_ICON_NAME_OVERRIDE.get(chain_data["chainId"], None)
                or shortcut.lower(),
                "eth",
                allow_generic=True,
            ),
            primary_color=ETH_PRIMARY_COLOR_MAPPING.get(
                chain_data["chainId"], "0x255770" if not is_testnet else "0x255770"
            ),
        )
